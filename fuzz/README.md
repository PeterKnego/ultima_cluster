...
| `uc_protocol_cluster_frame` | `uc_protocol::v2::frame::read_cluster_prefix` plus the kind-dispatched payload decoders (`config::decode_config`, `schedule::decode_schedule_table`, `settings::decode_settings`, `upgrade::decode_upgrade_pin`, `upgrade::decode_snapshot_report`) — the `CLUSTER` body every node decodes off the log (spec §4.3). |
| `uc_protocol_settings` | `uc_protocol::v2::settings` — the replicated settings record's codec (cluster-FSM spec §6): total on any slice, round-trips through re-encoding. |
| `uc_protocol_cluster_image` | `uc_protocol::v2::cluster_image` (plan 3, spec §4.8) — the cluster IMAGE codec, moved here out of `uc_node::cluster_fsm::{freeze, install_snapshot}` so it can be fuzzed without pulling in `uc_node`. Total, CRC-checked, exact framing; a decoded image round-trips through re-encoding. `uc_node_cluster_artifact` below still covers the same decoder through the real `ClusterFsm::install_snapshot` call site. |
...
| `uc_service_snapshot_envelope` | `uc_service::snapshots::decode_snapshot_envelope` — the 24-byte `ULTSNAP2` header every snapshot artifact file begins with (since `2.13.0`; a pre-`2.13.0` `ULTSNAP1` header is a named `Legacy` refusal, never decoded as version 0). A pure, total decoder, and the one check standing between a renamed, mis-copied, or wrong-version artifact and an install that would silently leave a span unapplied or cross a version boundary uncaught. |
