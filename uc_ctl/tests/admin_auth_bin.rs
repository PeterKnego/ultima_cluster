...
//! harness shape `uc_node/tests/admin_auth.rs` uses to drive the library
...
use uc_crypto::admin::{AdminKey, generate_key_file};
...
/// and `uc_node/tests/admin_auth.rs` rely on) under `root/<name>`.
