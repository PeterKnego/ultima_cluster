...
//! write: `ultima_journal::StableValue`'s two-slot rotation
...
/// what `uc2_node/tests/force_config.rs` pins byte-for-byte.
...
fn recover_locked(instance_dir: &Path) -> io::Result<(InstanceDir, NodeState, ConfigRecord, u64)> {
    ...
    //
    // Checked directly against `ultima_journal::journal::mod::Journal::open`
