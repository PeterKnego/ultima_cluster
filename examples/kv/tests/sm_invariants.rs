...
fn put_returns_position_as_version_and_get_sees_it() {
    ...
    assert_eq!(
        get(&sm, b"k"),
        GetReply::Found { version: pos(0), value: Bytes::from_static(b"v1") }
    );
...
fn delete_returns_old_version_then_not_found() {
    ...
    assert_eq!(write(&mut sm, pos(2), &wire::encode_delete(b"k")), WriteReply::NotFound);
...
fn cas_semantics() {
    ...
    assert_eq!(get(&sm, b"k"), GetReply::Found { version: pos(0), value: Bytes::from_static(b"a") });
    ...
    assert_eq!(get(&sm, b"k"), GetReply::Found { version: pos(2), value: Bytes::from_static(b"c") });
...
fn limits_are_enforced_in_apply_not_just_in_the_client() {
    ...
    assert_eq!(write(&mut sm, pos(0), &frame), WriteReply::BadRequest(wire::BAD_VALUE_LEN));
    ...
    assert_eq!(wire::COMMAND_BUDGET, uc_remote::engine::STANDARD_PAYLOAD - uc_service::SESSION_HEADER_LEN);
    assert_eq!(write(&mut sm, pos(1), &put), WriteReply::Ok { version: pos(1) });
    assert_eq!(write(&mut sm, pos(2), &cas), WriteReply::VersionMismatch { current: pos(1) });
}

#[test]
fn malformed_frames_are_bad_requests_not_panics() {
    let mut sm = KvSm::default();
    assert_eq!(write(&mut sm, pos(0), &[]), WriteReply::BadRequest(wire::BAD_TRUNCATED));
    assert_eq!(write(&mut sm, pos(1), &[9, wire::OP_PUT, 1, 0, b'k']), WriteReply::BadRequest(wire::BAD_FORMAT_VERSION));
    assert_eq!(write(&mut sm, pos(2), &[wire::FORMAT_VERSION, 200, 1, 0, b'k']), WriteReply::BadRequest(wire::BAD_UNKNOWN_OP));
    assert_eq!(write(&mut sm, pos(3), &[wire::FORMAT_VERSION, wire::OP_PUT, 5, 0, b'k']), WriteReply::BadRequest(wire::BAD_TRUNCATED));
    assert_eq!(write(&mut sm, pos(4), &[wire::FORMAT_VERSION, wire::OP_DELETE, 1, 0, b'k', b'x']), WriteReply::BadRequest(wire::BAD_TRAILING));
    assert_eq!(write(&mut sm, pos(5), &[wire::FORMAT_VERSION, wire::OP_CAS, 1, 0, b'k', 1, 2, 3]), WriteReply::BadRequest(wire::BAD_TRUNCATED));
    // Queries too.
    let q = query(&sm, &[]);
    assert_eq!(wire::decode_get_reply(&q).unwrap(), GetReply::BadRequest(wire::BAD_TRUNCATED));
    let q = query(&sm, &[wire::FORMAT_VERSION, 77]);
    assert_eq!(wire::decode_get_reply(&q).unwrap(), GetReply::BadRequest(wire::BAD_UNKNOWN_OP));
...
fn golden_script() -> Vec<Vec<u8>> {
    let mut v = Vec::new();
    for i in 0..50u32 {
        v.push(wire::encode_put(format!("key{i:03}").as_bytes(), &i.to_le_bytes()));
    }
...
fn golden_replay_pins_digest() {
    ...
    assert_eq!(a.recompute_digest(), da.digest, "incremental digest drifted from a full recomputation");
    // GOLDEN: recorded from the first green run; a change here is a wire or hash change.
    assert_eq!(da.digest, GOLDEN_DIGEST, "golden digest changed: 0x{:016x}", da.digest);
}
...
fn snapshot_round_trip_reproduces_state_and_cursor() {
    ...
    assert_eq!(landed, p);
    assert_eq!(fresh.last_applied(), sm.last_applied(), "cursor must come from the image, not the tag");
...
fn install_refuses_an_image_whose_cursor_is_not_below_the_tag() {
    ...
    KvSm::stream_snapshot(handle, &mut image).unwrap();
    let mut fresh = KvSm::default();
    assert!(fresh.install_snapshot(frozen_at, &mut &image[..]).is_err(), "cursor == tag is a mis-tag");
    assert!(fresh.install_snapshot(frozen_at - 32, &mut &image[..]).is_err());
    assert_eq!(fresh.last_applied(), None, "a refused install must not half-apply");
...
fn install_refuses_a_corrupt_image() {
    ...
    let mut fresh = KvSm::default();
    assert!(fresh.install_snapshot(frozen_at + 64, &mut &truncated[..]).is_err());
    let mut flipped = image.clone();
    let last = flipped.len() - 1;
    flipped[last] ^= 0x01;
    assert!(fresh.install_snapshot(frozen_at + 64, &mut &flipped[..]).is_err(), "digest must catch a flipped byte");
    let mut bad_version = image.clone();
    bad_version[0] = 99;
    assert!(fresh.install_snapshot(frozen_at + 64, &mut &bad_version[..]).is_err());
}
...
fn sessioned_replays_a_duplicate_frame_without_reapplying() {
    use uc_service::{SessionConfig, Sessioned, SESSION_HEADER_LEN, TAG_FRESH, TAG_REPLAYED};
    ...
    assert_eq!(get(sm.inner(), b"k"), GetReply::Found { version: pos(0), value: Bytes::from_static(b"v") });
    assert_eq!(sm.inner().last_applied(), Some(pos(0)));
    assert_eq!(sm.last_applied(), Some(pos(1)), "Sessioned tracks the dedup-only frame as its resume point");

    // A fresh seq from the same client is applied.
    let mut out3 = Vec::new();
    sm.apply(&mut ApplyCtx::for_sm::<KvSm>(pos(2)), &envelope(42, 2, &wire::encode_put(b"k", b"w")), &mut out3);
    assert_eq!(out3[0], TAG_FRESH);
    assert_eq!(get(sm.inner(), b"k"), GetReply::Found { version: pos(2), value: Bytes::from_static(b"w") });

    // Queries pass through the wrapper untouched (no envelope, no tag).
    let mut q = Vec::new();
    sm.query(&wire::encode_get(b"k"), &mut q);
    assert_eq!(wire::decode_get_reply(&q).unwrap(), GetReply::Found { version: pos(2), value: Bytes::from_static(b"w") });
}
