...
- **Hot paths untouched in cost**: a reader that becomes total (`Option`) may add one length compare, nothing else; no allocation, no copying. `uc_net`/`uc_log`/`uc_service` callers keep their existing guards (belt and braces) — do not remove a caller's guard because the callee is now total.
...
- **Naming**: fuzz targets are named `<crate>_<seam>` (`uc_protocol_datagram`, `uc_protocol_log_frame`, `uc_protocol_cnc`, `uc_remote_frame`, `uc_crypto_open`, `uc_crypto_handshake`, `uc_crypto_group_key`, `uc_crypto_admin`, `uc_journal_record`, `uc_journal_stable_value`, `uc_service_session`, `uc_node_toml`, `uc_gateway_toml`, `uc_node_http`); the nightly job is `fuzz`; the docs are `docs/security/threat-model.md`, `docs/security/attack-surface.md`, `docs/security/self-assessment.md`, `SECURITY.md` (repo root).
...
| `fuzz/corpus/<target>/` | committed seed corpora, generated by `fuzz/src/seeds.rs` (`cargo +nightly run -p uc_fuzz --bin seed-corpus`) |
...
| `uc_net/src/receiver.rs` (+ any other caller) | adapt to the `Option` readers (keep the existing length guards) |
| `uc_node/src/config_file.rs`, `uc_gateway/src/config_file.rs` | `pub fn parse_str(text: &str) -> Result<…>` (the pure inner of `load_from_path`) |
| `uc_node/src/obs/http.rs` | `#[cfg(any(test, fuzzing))] pub fn route_raw(buf: &[u8], sources: &ObsSources) -> (u16, &'static str, String)` (thin pub wrapper over the private `route`) + `ObsSources` constructible from test values (check what it holds; if it already is, nothing to add) |
...
- Modify: `uc_protocol/src/v2/datagram.rs` — `read_datagram_header`, `read_request_vote_body`, `read_vote_body`, `read_nak_body`, `read_status_body` return `Option<…>`; `uc_net/src/receiver.rs` (+ any other caller found by `cargo build --workspace`) adapt
- Test: `uc_protocol/src/v2/datagram.rs` unit tests (short-input → `None`), the fuzz target itself

**Interfaces:**
- Produces: `fuzz` crate named `uc_fuzz` with `[[bin]]` per target; `uc_fuzz::NoopSm` (a `uc_service::RawStateMachine` that ignores input) and `uc_fuzz::split(data: &[u8], n: usize) -> Vec<&[u8]>` helper; `scripts/fuzz_smoke.sh [SECS] [TARGET…]`.
...
uc_remote = { path = "../uc_remote" }
uc_crypto = { path = "../uc_crypto" }
uc_service = { path = "../uc_service" }
uc_node = { path = "../uc_node" }
uc_gateway = { path = "../uc_gateway" }
uc_journal = { path = "../uc_journal", package = "uc_journal" }  # check the real package name in uc_journal/Cargo.toml
...
Keep the doc comment honest: "Returns `None` on a short buffer; the receiver still guards the length before calling (belt and braces)". Then `cargo build --workspace --all-targets` and fix every caller — in `uc_net/src/receiver.rs` the pattern is:

```rust
let Some(h) = read_datagram_header(&buf[..n]) else { return None; }; // the existing len<16 guard stays above it
```

(`uc_sim`, `uc_node`, tests and examples may call these too — let the compiler list them.)

- [ ] **Step 6: Run tests** — `cargo test -p uc_protocol -p uc_net -p uc_sim` and `cargo clippy --workspace --all-targets -- -D warnings` — expected: all pass, zero warnings.
...
- [ ] **Step 11: Commit** — `git add -A fuzz scripts/fuzz_smoke.sh Cargo.toml uc_protocol uc_net <other callers>` ; `git commit -m "m12d: fuzz crate scaffold; uc_protocol datagram readers total on &[u8]; first target + seeds + fuzz_smoke.sh"`.
...
**Files:**
- Create: `fuzz/fuzz_targets/{uc_remote_frame,uc_crypto_open,uc_crypto_handshake,uc_crypto_group_key,uc_crypto_admin,uc_journal_record,uc_journal_stable_value,uc_protocol_cnc,uc_protocol_log_frame,uc_service_session}.rs`; `fuzz/corpus/<each>/*`
...
fuzz_target!(|data: &[u8]| {
    ...
    let peers = g.get_or_insert_with(|| /* build as in uc_crypto's handshake tests */ todo_build_peers());
    let _ = peers.on_message(uc_crypto::NodeId(1), kind, body, 1_000_000);
});
```

(Replace `todo_build_peers()` with the real construction copied from `uc_crypto`'s tests — the plan cannot see the private test helpers; the implementer copies them into `fuzz/src/lib.rs`.) `uc_crypto_group_key` does the same for `GroupPlane::on_key_message(from, body)` / `SharedTransport`'s `on_group_key_message`. Seeds: one genuine first handshake message produced by a second `Peers` (the initiator side), one kind-20 group-key message produced by the real rotation path, captured via the same test helpers.

- [ ] **Step 4: `uc_crypto_admin`** — round-trips `AdminMessage` canonical bytes and `verify`: split `data` into `app_id`, `instance_id`, `seq`, `nonce`, `op`, `id`, `ip`, `port`, `expiry_ns` with `uc_fuzz::split`, build an `AdminMessage`, compute `canonical_bytes()`, sign with a fixed `AdminKey` (use `AdminKey::from_bytes([9u8;32])` or whatever constructor exists — read `uc_crypto/src/admin.rs`), assert `verify(&key, &m, &tag) == true` and `verify` with a flipped tag byte `== false`. Assert `canonical_bytes().len() == 2 + app_id.len() + 16 + 8 + 8 + 4 + 4 + 4 + 2 + 8`. (This is a property target, not a parser — it guards the signed-tag layout against silent drift.)

- [ ] **Step 5: `uc_journal_record` and `uc_journal_stable_value`**

```rust
// uc_journal_record
#![no_main]
use libfuzzer_sys::fuzz_target;
use uc_journal::journal::segment::{decode_header, decode_record};
fuzz_target!(|data: &[u8]| {
    let _ = decode_header(data);
    let _ = decode_record(data, "fuzz-seg", 0);
});
// uc_journal_stable_value
#![no_main]
use libfuzzer_sys::fuzz_target;
...
- [ ] **Step 7: `uc_service_session`**

```rust
#![no_main]
use libfuzzer_sys::fuzz_target;
use uc_fuzz::NoopSm;
use uc_service::session::{Sessioned, SessionConfig};
use uc_service::RawStateMachine;
fuzz_target!(|data: &[u8]| {
    let parts = uc_fuzz::split(data, 3);
    ...
    let _ = uc_service::SnapshotStateMachine::install_snapshot(&mut sm, 3, &mut cur);
});
```
...
**Files:**
- Modify: `uc_node/src/config_file.rs` (`pub fn parse_str(text: &str) -> Result<(NodeConfig, StartupOptions), ConfigError>`; `load_from_path` = read file + `parse_str`; the `#[cfg(test)] load_str` helper becomes a thin alias or is deleted), `uc_gateway/src/config_file.rs` (`pub fn parse_str(text: &str) -> Result<EdgeConfig, ConfigFileError>`), `uc_node/src/obs/http.rs` (`#[cfg(any(test, fuzzing))] pub fn route_raw(buf: &[u8], sources: &ObsSources) -> (u16, &'static str, String)` + make `ObsSources` constructible from plain values under the same cfg if it is not already; `pub mod obs`/`pub mod http` visibility as needed — `cfg(fuzzing)` only)
- Create: `fuzz/fuzz_targets/{uc_node_toml,uc_gateway_toml,uc_node_http}.rs`, corpora
- Test: `uc_node/tests/daemon_refusals.rs` or `config_file.rs` unit tests — `parse_str` rejects an unknown key with the same `ConfigError` as `load_from_path` (one test each crate)

**Interfaces:**
- Produces: `uc_node::config_file::parse_str`, `uc_gateway::config_file::parse_str` (public, stable-ish: document "the file loader's pure inner; same errors"), `uc_node::obs::http::route_raw` (cfg-gated, not API).
...
#![no_main]
use libfuzzer_sys::fuzz_target;
fuzz_target!(|data: &[u8]| { if let Ok(s) = std::str::from_utf8(data) { let _ = uc_node::config_file::parse_str(s); } });
// uc_gateway_toml — same with uc_gateway::config_file::parse_str
// uc_node_http
#![no_main]
use libfuzzer_sys::fuzz_target;
use std::sync::OnceLock;
use uc_node::fuzz_seams::{route_raw, ObsSources};
...
- [ ] **Step 5: Run** — `scripts/fuzz_smoke.sh 60 uc_node_toml uc_gateway_toml uc_node_http` — clean; `cargo clippy --workspace --all-targets -- -D warnings`; `cargo test -p uc_node --lib` (obs tests).
...
```yaml
  fuzz:
    ...
    strategy:
      ...
      matrix:
        group:
          - "uc_protocol_datagram uc_protocol_log_frame uc_protocol_cnc uc_remote_frame"
          - "uc_crypto_open uc_crypto_handshake uc_crypto_group_key uc_crypto_admin"
          - "uc_journal_record uc_journal_stable_value uc_service_session"
          - "uc_node_toml uc_gateway_toml uc_node_http"
    steps:
