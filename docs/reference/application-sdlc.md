...
|---|---|---|---|---|
| New command variant (appended) | yes | **severe** — old replica cannot apply; leader acks anyway | permanent until S9 closes it | no |
| **Command variant inserted mid-enum** | yes | severe | **worst — measured silent misparse** | no |
| Field appended to an existing command | yes | severe; **silently drops the field** | permanent | no |
| **Fields reordered** | yes | **silent, and undetectable by any length check** | permanent | no |
| Integer field widened (`u32`→`u64`) | yes | **none measured** — varint makes it transparent | none | yes |
...
The measured cases behind the "silent" cells are the spec's Appendix A (`docs/superpowers/specs/2026-09-19-uc2-fsm-upgrade-lifecycle-design.md`): 12 probes of realistic schema changes, of which **5 decode successfully into a wrong value** with no error.
...
**The `IdGen` trap.** `uc_service/src/ids.rs` derives an id from the frame's position, the FSM's identity, and the **ordinal of the call within this `apply`**. So if the new version mints two ids where the old minted one — on a path that is otherwise behaviourally identical — every later id in that call differs, and a replica replaying the span under the new binary gets different ids than the old binary wrote. No single-version test can see this; it is a *diff* property. Diff replay counts each frame's `ctx.ids()` calls (`ApplyCtx::ids_calls`) and reports a change as a surface of its own — that is the check.
...
> `docs/BACKLOG.md` item 3). What 2.13.0 added is the pinned origin and the
...
2. `uc2ctl upgrade pin --row <R> --to <MAJOR.MINOR.PATCH> --origin <P> [--from <MAJOR.MINOR.PATCH>]` — appends the `UpgradePin` record. Refused by name if the row is undeclared (52), `--from` is not the row's current version (53), there is no complete set at `--origin` on this node (54), or the origin is not above the row's current pin (55).
