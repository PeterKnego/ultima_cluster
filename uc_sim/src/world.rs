...
//!   which ranks reports into `commit` — mirroring `uc_net`'s receiver reading
...
use uc_crypto::group::GroupPlane;
use uc_crypto::handshake::{HandshakeAction, Peers};
...
/// clamp that is STRICTLY STRONGER than what `uc_node` implements; `Mechanism`
/// models the REAL boolean intake-gate discipline `uc_node` actually runs** —
...
pub enum DataPlane {
    ...
    /// `uc_node` runs, so a bug in the gate's OPEN/CLOSE discipline (M4 C-1) is
    /// invisible under `Gated`. That is exactly why [`DataPlane::Mechanism`]
    /// exists.
    Gated,
    /// The REAL M3 receiver's behavior (the shipped `uc_net` FollowerReceiver).
    ...
    RawM3,
    /// The REAL mechanism `uc_node` implements: a per-node boolean `intake_gate`
    ...
    /// binary ships it.
    ///
    /// `reopen_guard: true` mirrors `uc_node` post-M4 (commit a8d98f4, the C-1
    ...
    ///
    /// `handle_keyed: true` mirrors `uc_node` post-Finding-#9 (lean LC2, gate
...
/// (`uc_crypto`'s own test helpers do exactly that, from inside
...
/// to compile — the same gap `uc_crypto/src/identity.rs`'s test helper
...
fn crypto_scratch_dir() -> std::path::PathBuf {
    ...
    let d = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../target/uc_sim_crypto_tests")
        .join("uc2-sim-crypto")
...
pub struct SimConfig {
    ...
    /// `refresh_durable()` call in `uc_node`.
    ///
...
pub enum Msg {
    ...
    /// reliable-UDP NAK path (`uc_net`'s receiver-driven retransmit
    ...
    /// (kind 18/19, `uc_crypto::handshake::Peers`) or an `HS_KEY`
    /// delivery/ack (kind 20, `uc_crypto::group::GroupPlane`) — routed
...
struct Node {
    ...
    /// as `uc_net`'s receiver reads `counters().durable` on its own thread.
    durable: u64,
    /// Issue #7: the consensus agent's absorbed copy of `durable` — the shadow
    /// `Consensus::durable_seen` keeps in `uc_node`, used only to notice a
    ...
    /// AppendPosition reported), `false` = CLOSED. Mirrors `uc_node`'s
    /// `intake_gate: Arc<AtomicBool>` — the ONLY divergence protection the binary
    /// ships (weaker than `Gated`'s structural clamp).
    intake_gate: bool,
    /// Shadow of the SM's adopted term (`uc_node::adopted_term`): the gate closes
...
/// clock reads) — `uc_crypto::handshake`/`uc_crypto::group`'s own design
...
/// own `uc_node` cadence is a separate, independently-tuned parameter
...
impl World {
    ...
    /// `uc_crypto::group::ACTIVATION_TIMEOUT_NS`) without a specific
    ...
    fn on_archive(&mut self, node: usize, now: u64, step: u64) -> Result<(), InvariantViolation> {
            ...
            && !self.archive_frozen.contains(&node)
        {
            ...
            if nd.append > nd.durable {
                ...
                // COUNTER, as `uc_net`'s receiver does.
        ...
        // Mechanism report FLOOR (mirrors `uc_net/src/receiver.rs` 1052-1078):
    ...
    /// step 2 in `uc_node` — poll the durable counter, feed `DurableAdvanced`
    ...
    fn on_restart(&mut self, node: usize, now: u64) -> Result<(), InvariantViolation> {
        ...
        // Rebuilt receiver (mirrors uc_node boot). Finding #5 (lean gate doc
    ...
    fn on_truncated_feedback(
        ...
        // fresh reconcile must complete first (mirrors uc_node::on_truncated).
    ...
    fn deliver(
        ...
        match msg {
            Msg::Data {
                ...
                if let Some(epoch) = epoch {
                    ...
                    if !openable {
                        ...
                        // `uc_crypto::group`'s own module doc is explicit this
            ...
            Msg::TermMap { term, entries } => {
                ...
                // Intake-gate CLEAN-RECONCILE reopen (mirrors uc_node::feed): a
    ...
    fn apply_action(
        ...
        match act {
            ...
            Action::BecomeLeader { term, base } => {
                ...
                // epoch on every BecomeLeader (`uc_crypto::rotation`'s
            ...
            Action::Truncate { epoch, to, new_map } => {
                ...
                if mechanism {
                    ...
                    // decision for this term (mirrors uc_node::exec Truncate).
                    //
                    // PERSIST-BEFORE-TRUNCATE (uc_node): the PRUNED map (`new_map`,
    ...
    /// lives in `uc_consensus` behind the same feature.
    #[cfg(feature = "mutation-testing")]
    ...
    /// `sealing_epoch` vs `schedule` in `uc_crypto::group::GroupPlane`).
    ...
    /// (`uc_crypto::rotation`). A no-op if the minter is not
    /// crypto-enabled.
    ...
    /// A node's data-plane term handle shadow (`uc_node::adopted_term` ==
