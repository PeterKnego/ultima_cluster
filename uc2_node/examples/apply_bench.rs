...
//! — plus a driver thread that appends frames through `uc2_log::Appender` and
//! plays archive + consensus (`durable = commit = append`, published every
//! `--batch` frames), paced so `append − min(applied) ≤ --window`; N
//! `uc2_service` attaches apply a raw-tier counting state machine. No sink:
...
//!
//! ```text
//! cargo run -p uc2_node --release --example apply_bench -- --root /home/claude/apply-bench --fsms 2 --mode bounded --secs 6
//! cargo run -p uc2_node --release --example apply_bench -- --root /home/claude/apply-bench --fsms 2 --mode lockstep --secs 6
//! ```
...
use uc2_log::cnc::{CncMeta, CncPage, unpack_service_status};
use uc2_service::{RawStateMachine, ServiceBuilder, ServiceConfig};
