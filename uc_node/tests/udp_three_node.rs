//! 3-node shmem cluster test over the UDP inter-node transport (Task 13).
//!
//! Identical in shape to `m3_three_node_shmem.rs` but selects
//! `Transport::Udp(UdpTuning::default())` instead of QUIC. Proves a real
//! 3-node openraft cluster elects a leader and replicates/converges over the
//! Aeron-style reliable-UDP channel.
//!
//! Brings up three nodes, each in `IpcMode::Shmem` paired with its own
//! `uc_service::Service`, all running in one tokio runtime.
//!
...
//! needs peers 2 and 3 to have their UDP servers up. Each node's
...
use uc_node::{
    ...
    PeerSeed, RaftTuning, ServiceRingConfig, TlsConfig, Transport, UdpTuning,
};
...
#[tokio::test]
async fn udp_three_node_replicates() {
    ...
    let app_id = "udp-three-node".to_string();
    ...
    for (i, addr) in addrs.iter().enumerate() {
        ...
        let cfg = NodeConfig {
            ...
            transport: Transport::Udp(UdpTuning::default()),
