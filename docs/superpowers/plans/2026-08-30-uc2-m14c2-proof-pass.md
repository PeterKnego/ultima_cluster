...
**Architecture:** One harness extension (`uc2_node/tests/lincheck_v2/mod.rs` learns a second FSM per node, a `Slow<SM>` wrapper and a `submit_all_cmd` that records into two histories) unlocks three tiers; the crashtest gets the same via a shared `ServicesConfig::from_cli` and two new process-level tests; every two-FSM capstone asserts per-FSM linearizability with the untouched `uc-lincheck` checker **plus** the replication-equivalence oracle (every `submit_all`'s responses byte-equal), and each new oracle is shown to bite on an injected divergence. The lockstep question is answered by a reproducible dev-box experiment with a pre-committed decision rule before any code changes. No wire/cnc change; 2.8.1 is API-compatible with 2.8.0.

**Tech Stack:** Rust 1.96 workspace; `uc2_node` test harness (`lincheck_v2`), `uc-lincheck` (WGL checker, `RegisterSm`, `EdnRecorder`), `uc2_client::Client`, `uc2_service::{StateMachine, SnapshotStateMachine}`; `examples/uc2-crashtest` (real processes, `hard-crash-tests` feature); `scripts/elle_check.sh` + vendored `elle-cli`; `uc2_node/examples/apply_bench` + `taskset`/`stress-ng`; Python 3 fleet drivers under `bench-infra/scripts/`.
...
| `uc2_node/src/services.rs` | `ServicesConfig::from_cli(ids, fsm_lag)` + tests | 1 |
| `uc2_gateway/examples/m12_gate.rs`, `examples/uc2-crashtest/src/bin/uc2-crashtest-node.rs` | use `from_cli`; node bin gains `--services`/`--fsm-lag` | 1 |
| `uc2_node/tests/lincheck_v2/mod.rs` | `FsmSet`, `Slow<SM, MS>`, `Corrupt<SM>`, second service per slot, `submit_all_cmd`, `service_applied`, respawn of both | 2 |
| `uc2_node/tests/lin_v2.rs` | `two_fsm_bounded`, `two_fsm_lockstep`, `two_fsm_oracle_bites`, `two_fsm_slow`, `two_fsm_slow_lockstep` | 3, 4 |
| `uc2_node/tests/lin_partition_v2.rs` | `minority_partition_and_heal_two_fsm` | 5 |
| `examples/uc2-crashtest/tests/common/mod.rs`, `tests/hard_crash.rs` | `spawn_service_id`, `spawn_node_with_services`, `submit_all_cmd`, two tests | 6 |
| `uc2_node/tests/elle_v2.rs`, `scripts/elle_check.sh` | `elle_quiet_two_fsm`, pass registered | 7 |
| `scripts/lockstep_oversub.sh`, `docs/benchmarks/uc2-m14c2-lockstep-oversubscription-2026-08-30.md`, maybe `uc2_service/src/apply.rs` | the experiment, its record, the conditional fix | 8 |
| `bench-infra/scripts/m12_fleet_gate.py`, `m14_fleet_gate.py`, `m14_ab_27_vs_28.py` | `CPUAffinity` per unit, `--pin` | 9 |
| `uc2_net/src/{sender,receiver}.rs` + tests, `uc2_node/src/obs/metrics.rs` | deferrals (net) | 10a |
| `uc2_service/src/apply.rs`, `uc2_node/src/node.rs`, `uc2_node/tests/learner.rs`, `uc2ctl/src/main.rs`, docs | deferrals (service/node/ctl/docs) | 10b |
| `uc2_node/tests/lin_v2.rs` (or `purge_safety.rs`) | `snapshot_install_needs_purge` pin | 11 |
...
**Files:**
- Modify: `uc2_node/src/services.rs` (after `from_ids`), `uc2_gateway/examples/m12_gate.rs` (`services_from_flags`), `examples/uc2-crashtest/src/bin/uc2-crashtest-node.rs` (args + `NodeConfig.services`)
- Test: `uc2_node/src/services.rs` `#[cfg(test)]` module (exists — add to it)
...
- [ ] **Step 2: Run to verify they fail** — `cargo test -p uc2_node --lib services::tests::from_cli 2>&1 | tail -3` → `no function or associated item named from_cli`.
...
Then in `m12_gate.rs` replace `services_from_flags`'s body with `ServicesConfig::from_cli(services, fsm_lag).map_err(anyhow::Error::msg)` (keep the function so its three tests still compile; they assert on the same prefixes). In `uc2-crashtest-node.rs` add to `Args`:
...
and in the `NodeConfig` literal: `services: uc2_node::ServicesConfig::from_cli(args.services.as_deref(), args.fsm_lag.as_deref()).unwrap_or_else(|e| { eprintln!("{e}"); std::process::exit(2) }),`.

- [ ] **Step 4: Verify** — `cargo test -p uc2_node --lib services:: 2>&1 | tail -3` (all pass); `cargo test -p uc2_gateway --example m12_gate 2>&1 | tail -2` (7 pass); `cargo build -p uc2-crashtest 2>&1 | tail -1`; `cargo run -p uc2-crashtest --bin uc2-crashtest-node -- --help | grep -c 'services\|fsm-lag'` → 2; clippy clean.

- [ ] **Step 5: Commit** — `git commit -am "feat(node): ServicesConfig::from_cli — the shared --services/--fsm-lag parser; m12_gate and uc2-crashtest-node use it (M14c2 T1)"`
...
**Files:**
- Modify: `uc2_node/tests/lincheck_v2/mod.rs` (`ClusterCfg`, `NodeSlot`, `LinClusterV2`, `make_config`, `spawn_service`, `start_cfg`, `kill_and_restart_leader`, `supervise_services`, `crash_and_restart_leader_service`, `crash_and_restart_random_follower_service`, the spare-node spawn if it spawns a service, `stop`; new `submit_all_cmd`, `service_applied`, `Slow`, `Corrupt`)
- Test: a smoke test appended to `uc2_node/tests/lin_v2.rs`

**Interfaces:**
- Consumes: `uc2_client::Client::submit_all<C, R>(&self, &C) -> Result<Vec<(u8, R)>, ClientError>` (`uc2_client/src/client.rs:140`); `uc2_log::cnc::CncPage::open_file(&Path, app_id)` + `.service_slot(i).applied.load_acquire()`; `ServiceConfig::service_id(u8)`; `ServicesConfig::from_ids(&[0, 1], Some(lag))`.
- Produces:
  ```rust
  #[derive(Clone, Copy)] pub enum FsmSet { Single, Two { lag: uc2_node::FsmLag } }
  ...
  ```
- Design note: `NodeSlot<SM, SM1>` gains `service1: Option<uc2_service::Service<SM1>>`, `None` under `Single`. Every place that takes/crashes/respawns `service` does the same for `service1` (node-before-service teardown order kept: node, service, service1).
...
fn two_fsm_smoke() {
    let _g = serialize();
    let dir = tempdir();
    let ccfg = ClusterCfg { services: lincheck_v2::FsmSet::Two { lag: uc2_node::FsmLag::Bounded(64 * 1024) }, ..ClusterCfg::default() };
...
- [ ] **Step 2: Run to verify it fails** — `cargo test -p uc2_node --test lin_v2 two_fsm_smoke 2>&1 | grep -E 'error\[' | head -3` → `no variant or associated item named Two` / `no field services`.
...
`make_config` gets `services: uc2_node::ServicesConfig` from the caller:
```rust
fn services_config(ccfg: ClusterCfg) -> uc2_node::ServicesConfig {
    match ccfg.services {
        FsmSet::Single => uc2_node::ServicesConfig::default(),
        FsmSet::Two { lag } => uc2_node::ServicesConfig::from_ids(&[0, 1], Some(lag)).expect("ids 0,1"),
    }
}
```
...
```rust
fn spawn_service<SM: SnapshotStateMachine + Default>(dir: &Path, snapshot_interval_bytes: u64, id: u8) -> uc2_service::Service<SM> {
...
fn spawn_service1<SM1: SnapshotStateMachine + Default>(dir: &Path, ccfg: ClusterCfg) -> Option<uc2_service::Service<SM1>> {
...
impl<SM: uc2_service::StateMachine, const MICROS: u64> uc2_service::StateMachine for Slow<SM, MICROS> {
    ...
    fn query(&self, q: SM::Query) -> SM::QueryResponse { uc2_service::StateMachine::query(&self.0, q) }
    fn last_applied(&self) -> Option<u64> { uc2_service::StateMachine::last_applied(&self.0) }
}
impl<SM: uc2_service::SnapshotStateMachine, const MICROS: u64> uc2_service::SnapshotStateMachine for Slow<SM, MICROS> {
    type SnapshotHandle = SM::SnapshotHandle;
    fn freeze(&self) -> Result<(SM::SnapshotHandle, u64), uc2_service::SnapshotError> { self.0.freeze() }
    fn stream_snapshot(h: SM::SnapshotHandle, dst: &mut dyn std::io::Write) -> Result<(), uc2_service::SnapshotError> { SM::stream_snapshot(h, dst) }
    fn install_snapshot(&mut self, position: u64, src: &mut dyn std::io::Read) -> Result<u64, uc2_service::SnapshotError> { self.0.install_snapshot(position, src) }
}
...
impl uc2_service::StateMachine for Corrupt<RegisterSm> {
    ...
    fn query(&self, q: ()) -> Option<u64> { uc2_service::StateMachine::query(&self.0, q) }
    fn last_applied(&self) -> Option<u64> { uc2_service::StateMachine::last_applied(&self.0) }
}
impl uc2_service::SnapshotStateMachine for Corrupt<RegisterSm> { /* forward all three exactly as Slow does */ }
```
...
```rust
    ...
    pub fn service_applied(&self, node: usize, id: u8) -> u64 {
        let cnc = uc2_log::cnc::CncPage::open_file(&self.nodes[node].instance_dir.join("cnc2.dat"), APP).expect("open cnc");
        cnc.service_slot(id as usize).applied.load_acquire()
    }
```

- [ ] **Step 4: Verify** — `cargo test -p uc2_node --test lin_v2 two_fsm_smoke 2>&1 | tail -3` → ok; then the untouched suites: `cargo test -p uc2_node --test lin_v2 --test lin_partition_v2 --test elle_v2 2>&1 | grep -E '^test result'` (elle's are `#[ignore]`d — 0 run is fine); `cargo test --workspace 2>&1 | grep -E '^test result' | grep -v ' 0 failed'` → no lines; clippy clean.
...
**Files:**
- Modify: `uc2_node/tests/lincheck_v2/mod.rs` (new `worker2`, `spawn_workers2`), `uc2_node/tests/lin_v2.rs` (three tests)
...
#[test] fn two_fsm_bounded()  { run_two_fsm("two_fsm_bounded",  uc2_node::FsmLag::Bounded(64 * 1024), std::env::var("LIN_SEED").ok().and_then(|s| s.parse().ok()).unwrap_or(0x14c2)); }
#[test] fn two_fsm_lockstep() { run_two_fsm("two_fsm_lockstep", uc2_node::FsmLag::Lockstep,            std::env::var("LIN_SEED").ok().and_then(|s| s.parse().ok()).unwrap_or(0x14c3)); }
...
- [ ] **Step 2: Run to verify they fail** — `cargo test -p uc2_node --test lin_v2 two_fsm 2>&1 | grep -E 'error\[' | head` → `spawn_workers2` not found.
...
- [ ] **Step 4: Verify** — `cargo test -p uc2_node --test lin_v2 two_fsm 2>&1 | grep -E '^test |test result'` → `two_fsm_smoke`, `two_fsm_bounded`, `two_fsm_lockstep` ok; `two_fsm_oracle_bites` ok (should_panic satisfied). Run each seeded twice (`LIN_SEED=1`, `=2`). Workspace suite green; clippy clean.
...
**Files:**
- Modify: `uc2_node/tests/lin_v2.rs`
...
fn run_two_fsm_slow(label: &str, lag: uc2_node::FsmLag, seed: u64) {
    ...
    let samples = {
        ...
        std::thread::spawn(move || {
            let cnc = uc2_log::cnc::CncPage::open_file(&dir0.join("cnc2.dat"), "uc2-lincheck-v2").expect("cnc");
            let mut v = Vec::new();
...
- [ ] **Step 3: Verify** — `cargo test -p uc2_node --test lin_partition_v2 two_fsm 2>&1 | tail -3` ok; the four existing scenarios still ok; clippy clean.
...
**Files:**
- Modify: `examples/uc2-crashtest/tests/common/mod.rs` (`spawn_service_id`, `spawn_node_with_services`), `examples/uc2-crashtest/tests/hard_crash.rs` (`submit_all_cmd`, `worker2`, two tests)
...
- [ ] **Step 2: Run to fail** — `cargo test -p uc2-crashtest --features hard-crash-tests --test hard_crash two_fsm 2>&1 | grep 'error\[' | head -3` → missing helpers.
...
- [ ] **Step 4: Verify** — `cargo test -p uc2-crashtest --features hard-crash-tests --test hard_crash 2>&1 | grep -E '^test |test result'` → all (old + 2 new) ok, twice; clippy `-p uc2-crashtest --features hard-crash-tests` clean.
...
**Files:**
- Modify: `uc2_node/tests/elle_v2.rs` (`elle_worker2`, `run_pass2`, `elle_quiet_two_fsm`), `scripts/elle_check.sh` (default passes)
...
- Possibly modify: `uc2_service/src/apply.rs` (only if the decision rule says defect)

**Interfaces:**
- Consumes: `uc2_node/examples/apply_bench` (`--root --fsms --mode --secs --warmup-secs`; prints `fsm={id} applied_frames/s=… lag_waits=…` and `hop: min applied_frames/s=…`), `taskset`, `stress-ng` (both present on the box), the constants `LAG_WAIT_SPINS=256`, `LAG_WAIT_YIELDS=2048` (`apply.rs:527-528`), `APPLY_IDLE = Sleep(50 µs)` (`uc2_service/src/lib.rs:81`).
...
- [ ] **Step 4: If defect** — land the smallest passing variant in `apply.rs` with its doc comment updated (the M14a paragraph at `apply.rs:621-633` gets a second paragraph naming this experiment), `cargo test -p uc2_service` and the full suite green, `two_fsm_slow_lockstep` (Task 4) still green. **If envelope** — add the sentence with the number to `docs/reference/configuration.md`'s `[services]` lockstep paragraph and `docs/reference/limits.md`.
...
### Task 10a: M14c deferrals — `uc2_net`

**Files:** `uc2_net/src/sender.rs` (`try_open_snap_session` at `:1016`), `uc2_net/src/receiver.rs` (`snap_chunk` `:1770`, `snap_upkeep` `:1981`, intake), `uc2_node/src/obs/metrics.rs`, `uc2_net/tests/snapshot_session.rs`
...
- [ ] **Step 2: Receiver** — an intake timeout: an intake with no chunk for 60 s (constant `SNAP_INTAKE_TIMEOUT_NS`) is abandoned (`.part` unlinked, counted `uc2_snapshot_intake_abandoned_total`); an undecodable BEGIN from the live peer counted once per session (`uc2_snapshot_begin_undecodable_total`); `snap_chunk`'s `seek`/`write_all` failure counted in the existing `snap_intake_io_failures` and logged once; `snap_upkeep` re-drives at most once per `SNAP_REDRIVE_INTERVAL_NS = 250 ms` (a `last_publish_try_ns` field). Tests in `uc2_net/tests/snapshot_session.rs` for the timeout and the cadence (virtual `now` is already injectable there).
- [ ] **Step 3: Verify** — `cargo test -p uc2_net -p uc2_node 2>&1 | grep -E '^test result' | grep -v ' 0 failed'` → none; the new counters appear in `uc2_node/src/obs/metrics.rs`'s `CONTRACT_SERIES` and `every_contract_series_is_present` passes; clippy clean.
...
**Files:** `uc2_service/src/apply.rs` (`lag_waits` at `:329-333`), `uc2_node/src/node.rs` (`note_service_transitions` `:2889`), `uc2_node/tests/learner.rs` (`fresh_learner_joins_a_purged_two_fsm_leader_and_both_fsms_converge` `:641`), `uc2ctl/src/main.rs` (`:535`), `docs/how-to/diagnose-a-node.md`, `docs/reference/semver-policy.md`, `packaging/prometheus/uc2-alerts.yml`
...
**Files:**
- Modify: `uc2_node/tests/lin_v2.rs` (or a new `uc2_node/tests/snapshot_restart.rs` including the harness module the same way)
...
```
- [ ] **Step 3: Verify** — `cargo test -p uc2_node --test lin_v2 snapshot_restart 2>&1 | tail -3` ok; then flip the expectation to prove it bites (`(false) == 1` must fail) and flip back.
...
- [ ] **Step 1** — version `2.8.0 → 2.8.1` everywhere `cut-a-release.md` §1 lists (same `grep`/`sed` sweep as M14d T8, historical mentions kept); `cargo metadata … uc2_node … .version` → `2.8.1`; build + clippy + `cargo package -p uc_protocol --allow-dirty --no-verify`.
