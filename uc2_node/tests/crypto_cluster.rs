...
/// `uc2_crypto::identity::Identity` (the crate's own accessor — this test has
...
fn write_crypto_material(dir: &Path, n: usize) -> Vec<(PathBuf, PathBuf)> {
    ...
    for i in 0..n {
        ...
        publics.push(uc2_crypto::identity::Identity::load(&key_path).unwrap().public_bytes());
...
/// (fixed in `uc2_crypto::group::GroupPlane::mint` — see its
