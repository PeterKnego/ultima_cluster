...
| 8 | Who drives the clean-room session | A **separate Claude Code session the maintainer starts in a separate directory** (`~/ultima/kv-dogfood`, own `CLAUDE.md`, own memory, no path to this repo). Its transcript under `~/.claude/projects/` is what gets audited. Same for the operator. |
