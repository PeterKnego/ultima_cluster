...
| f | learner declared `{0,1}` joins a purged two-FSM leader under load | **≤ 60 s**, `snapshot_session_refusals() == (0, 0)` on every node, both artifacts present on the learner, row c on the learner, and ≥ 1 snapshot install observed on the learner (`snapshot_installed` in its node log) | |
