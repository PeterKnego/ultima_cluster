...
- **Frozen:** the journal public API surface (signatures in
  `ultima_journal/src/journal/mod.rs`, `lib.rs` re-exports), `uc_autobench/**`,
  all tests and benches.
- **Floor:** `cargo test -p uc_autobench --test journal_torture` green.
- **Gates:**
  - Gate A: `cargo test -p ultima-journal` + `cargo test -p uc_node` (the
    journal's in-cluster consumer).
  - Gate B (manual): cluster `shmem-e2e` **p50 median-of-N** within the
    noise-aware band (≤50% regression vs champion p50) — p99 is recorded but
    NOT gated. NOTE: unlike the shmem task, run-iter does **not** auto-run this
    gate for journal-commit — the task's primary metric is a *maximize*
    (throughput), which run-iter's lower-is-better gate-trigger can't reason
    about, so its `task_spec` entry sets `gate_bin: None`. Run shmem-e2e by hand
    (median of N, default 5, `AUTOBENCH_E2E_SAMPLES` to override) for the cluster
    cross-check. Wiring an auto gate would require making run-iter's gate-trigger
    direction-aware.
...
- `write_only_p50_ns`: p50 of the page-cache append alone (encode + `write_all`,
  ...
  for all isolated/prealloc metrics lives in `uc_autobench/baselines/journal-commit.json`,
