...
-- proposer's own `hasAdopted i i` is the `propose`-time corner: `propAfterE i := holdsE i`
...
-- and the meeting member is necessarily a holder (which is exactly the step the retired
-- clause's "every quorum contains a holder" form could not take).
invariant [cfgq_witness]
  cfgCommitted D → quorumOf (cfgQ D) D
invariant [cfgq_holders]
  (cfgCommitted D ∧ cfgBacked D) →
    (∀ V, qmember V (cfgQ D) → (holdsE V ∧ tot.le (gotEAt V) (cfgCommitTerm D)))

-- ---- SESSION 8, run 41's CTI: two GHOST-SOUNDNESS clauses (T39/T40) ----
-- Same class as `commit_leader_unique` / `commit_leader_only_after_commit`: without them the
-- solver invents pre-states no action can produce. Run 41's `commitEntry` CTI is exactly that
-- — a node with `propAfterE` set that has never held E (and a `cfgBacked` config that never
-- committed).
-- T39 — `propAfterE` is written ONLY by `propose i`, as `propAfterE i := holdsE i`, and
-- `holdsE` is monotone (no action ever clears it), so the flag cannot outlive the fact.
invariant [propafter_holds]
  propAfterE I → holdsE I
-- T40 — `cfgBacked D` and `cfgCommitted D` are written in the SAME `commitCfg` step, for the
-- same `D = proposedC i`; neither is ever cleared.
invariant [cfgbacked_committed]
  cfgBacked D → cfgCommitted D
