...
| coordinated snapshots | no gate doc of its own — rows f/g/h above | pending. The three rows the feature owed now exist in the time-and-timers gate, with pre-committed bars; two (g/h) need the fleet and neither has run; row f needs only a quiet host and has a dev-box smoke reading only (`inconclusive (noisy run)`, not a gate result) |
