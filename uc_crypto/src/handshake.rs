...
//! only as `now_ns`. That is what lets `uc_sim` (T13) adjudicate the handshake
...
//! semantic content is the header, and `uc_net`'s receiver reads a follower's
...
mod tests {
    ...
    fn scratch() -> PathBuf {
        ...
        let d = std::env::var("CARGO_TARGET_TMPDIR")
            .map(PathBuf::from)
            .unwrap_or_else(|_| {
                PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../target/uc_crypto_tests")
            })
