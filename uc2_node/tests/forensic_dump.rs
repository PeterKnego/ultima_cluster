...
//!
//! ```bash
//! UC2_FORENSIC_DIR=/path/to/kept-root cargo test -p uc2_node --release \
//!     --test forensic_dump -- --ignored --nocapture
//! ```

use ultima_journal::{Journal, JournalConfig};
...
fn forensic_dump() {
    ...
    for n in 0..8 {
        ...
        println!("===== n{n} =====");
        match uc2_log::state::NodeState::open(&dir.join("state")) {
            Ok(st) => {
