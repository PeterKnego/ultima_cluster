...
/// Jumbo spec §8: the remedy clause the notification carries. This crate's text
/// is its OWN wording, not a copy — it omits `uc_client`'s "this cluster carries
/// it" clause, because protocol v1 never tells a remote client the cluster's
/// ceiling. `uc_client::engine::JUMBO_REMEDY` carries the full list of the five
/// places this advice appears and the two reasons they stay hand-written (no
/// crate can host a constant all five reach — this one takes `uc_protocol` as a
/// dev-dependency only — and the texts differ per tier on purpose). Every copy is
/// pinned by a test asserting the `force_jumbo_frames` token, so the wording
