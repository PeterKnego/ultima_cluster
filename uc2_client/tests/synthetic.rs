...
//! Hand-rolled instance dirs (cnc page + ring files, no real `uc2_node`/
//! `uc2_service` process) exercising client-side mechanics that a real node
...
//!
//! These tests only need `uc2_log` + `uc_protocol` (already regular deps of
//! `uc2_client`) — they build the same well-known files
//! (`uc2_node::InstanceDir`'s contract: `cnc2.dat`, `ingress.ring`,
...
fn injected_retry_frame_is_delivered_as_retry_error() {
    ...
    let mut producer = BroadcastRing::open(&dir.path().join("egress_service.broadcast"))
        .unwrap()
        .producer();
