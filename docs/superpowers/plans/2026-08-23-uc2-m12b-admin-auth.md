...
- No consensus change, no wire-protocol change, **no cnc layout change outside the reserved band** (`3904..4096`) — the new line occupies `3904..3968`; pin it in BOTH `uc_protocol` and `uc_log` with offset-assertion tests (CLAUDE.md rule). No new remote-admin surface: admin stays local through the cnc slot; the gateway carries no admin ops.
...
- Verification before every commit: `cargo clippy --workspace --all-targets -- -D warnings` (plus `-p uc_service --features ultima_db`, `-p uc_service --features apply-profile`, `-p uc_gateway --features test-util`), `cargo test -p <crate>` for the touched crates, and at the end `cargo test --workspace --exclude uc_node` + the `uc_node` fast set (`--lib --test smoke --test failover --test learner --test purge_safety --test query_barrier`) + `--test reconfig` (it drives the admin slot).
- Tests write only under `CARGO_TARGET_TMPDIR` (`tempdir()` helpers as in `uc_node/tests/lincheck_v2/mod.rs`); never `/tmp`. SPDX header on every new file.
- The audit log is append-only; **record before respond**; one `fsync` per record (admin ops are rare — document the cost).
- Key files: 32 raw bytes, refused if group/world-accessible (`mode & 0o077 != 0`, the same rule `uc_crypto::identity::Identity::load` applies) — factor that check into one shared helper.
...
| `uc_log/src/cnc.rs` | `AdminAuth { tag, expiry_ns, key_name_hash }`, `read_admin_auth`/`write_admin_auth`, roundtrip+offset-pin test |
| `uc_crypto/src/admin.rs` (new) | `AdminKey`, `AdminPolicy`, `fnv1a64`, canonical message, `sign`/`verify`, `load_key_file`, `generate_key_file`, shared `check_key_file_perms` (moved out of `identity.rs`) |
| `uc_node/src/node.rs` | `StartOpts`, `Node::start_with`, `handle_admin` verify-first + reason codes 20–23, `on_config_proposal` audit hook |
| `uc_node/src/audit.rs` (new) | `AuditLog` (O_APPEND + fsync), `AuditRecord`, `AuditOutcome`; `obs::log::format_line` refactor |
| `uc_node/src/config_file.rs`, `src/bin/uc2-node.rs` | `CryptoSection.enabled`, `AdminSection`, `ConfigError::{CryptoChoiceRequired, AdminChoiceRequired, AdminKeys}`, `StartupOptions.admin`, policy build in the daemon |
| `uc_ctl/src/main.rs` | `--admin-key`/`--admin-key-name` on `CommonArgs`, `gen-admin-key`, `audit`, reason strings 20–23 |
| `uc_ctl/tests/admin_auth_bin.rs` (new) | end-to-end: in-process `Node::start_with(Hmac)` + the real `uc2ctl` binary |
| `uc_node/tests/admin_auth.rs` (new) | in-process verify/audit tests |
...
- Modify: `uc_log/src/cnc.rs` (structs near `AdminReq`/`AdminResp` ~162-180; accessors near 512-600; tests near 1017-1060)
...
- [ ] **Step 1: Write the failing tests.** In `uc_log/src/cnc.rs` tests, next to `free_disk_bytes_roundtrip_and_offset_pin`:
...
- [ ] **Step 2: Run** — `cargo test -p uc_log admin_auth` → FAIL (unresolved); `cargo test -p uc_protocol` → FAIL.
- [ ] **Step 3: Implement** — `uc_protocol`: the const with a doc comment in the style of `CNC_OFF_ADMIN_REQ` (layout per byte, writer = uc2ctl, reader = consensus agent, "Next free reserved-band offset after this line: 3968") and `const _: () = assert!(CNC_OFF_ADMIN_AUTH + 64 <= CNC_PAGE_LEN);`. `uc_log`: the struct + `ZERO`/`is_zero`; `write_admin_auth` writes tag, expiry LE, hash LE as plain byte stores (no seq word of its own — ordering comes from `req.seq`); `read_admin_auth` plain loads. Keep the module's existing raw-slice style.
- [ ] **Step 4: Run** — both crates' tests PASS; `cargo clippy --workspace --all-targets -- -D warnings` clean.
- [ ] **Step 5: Commit** — `git commit -am "feat(cnc): CNC_OFF_ADMIN_AUTH = 3904 — admin-request HMAC line, pinned in uc_protocol and uc_log"`.

---

### Task 2: `uc_crypto::admin` — keys, canonical message, sign/verify, key files

**Files:**
- Create: `uc_crypto/src/admin.rs`
- Modify: `uc_crypto/src/lib.rs` (`pub mod admin;`, `CryptoError` variants), `uc_crypto/src/identity.rs` (use the shared perms helper), `uc_crypto/Cargo.toml` (`hmac = "0.12"` direct dep; add to `[workspace.dependencies]` as `hmac = "0.12"` and reference `{ workspace = true }`)
...
- [ ] **Step 2: Run** → FAIL. **Step 3: Implement** (canonical bytes exactly as documented; `sign` via `Hmac::<Sha256>::new_from_slice`; `verify` via `verify_slice`; `AdminKey` zeroizes; `identity.rs` calls `check_key_file_perms`). **Step 4: Run** `cargo test -p uc_crypto` PASS; clippy clean.
- [ ] **Step 5: Commit** — `git commit -am "feat(crypto): uc_crypto::admin — named HMAC-SHA256 admin keys, canonical request message, key files (shared 0600 rule)"`.
...
**Files:**
- Modify: `uc_node/src/node.rs` (`Node` fields ~344/1707; `start`/`start_with_socket` ~427-435; `handle_admin` 3452-3480; the admin-poll step ~2011-2014)
- Modify: `uc_node/src/lib.rs` (re-export `uc_crypto::admin::{AdminPolicy, AdminKey}` and `StartOpts`)
- Test: `uc_node/tests/admin_auth.rs` (new)
...
- Produces:
  ...
  // reason codes (uc_node::admin_reason, pub consts, documented next to REASON_MALFORMED_OP = 11):
...
fn verify_admin(&self, req: &AdminReq) -> Result<Option<String> /* actor key name */, u32> {
    match &self.admin {
        ...
        AdminPolicy::Hmac { keys, ttl } => {
            ...
            if !uc_crypto::admin::verify(key, &m, &auth.tag) { return Err(REASON_AUTH_BAD_TAG); }
...
- [ ] **Step 1: Write the failing tests** — `uc_node/tests/admin_auth.rs`, single-node in-process cluster (copy `make_config` from `lincheck_v2/mod.rs` with `n = 1`, `tempdir()` under `CARGO_TARGET_TMPDIR`), a helper that mirrors `uc2ctl`'s flow without the binary:
...
fn admin_request(cnc: &CncPage, op: u32, id: u32, addr: (u32, u16), auth: Option<(&AdminKey, Duration /*ttl*/ , Option<[u8;32]> /*override tag*/)>) -> AdminResp {
    ...
    match auth {
        ...
        Some((key, ttl, tag_override)) => {
            ...
            let tag = tag_override.unwrap_or_else(|| uc_crypto::admin::sign(key, &m));
...
**Files:**
- Create: `uc_node/src/audit.rs`
- Modify: `uc_node/src/obs/log.rs` (extract `pub(crate) fn format_line(level, event, fields) -> String`; `emit` calls it), `uc_node/src/node.rs` (`audit` field + calls at every `write_admin_reply` and in `on_config_proposal`), `uc_node/src/lib.rs` (`pub mod audit;`)
- Modify: `docs/reference/instance-directory.md` (durability table: `audit.jsonl` = must survive)
- Test: `uc_node/tests/admin_auth.rs` (extend), `uc_node/src/audit.rs` unit tests
...
- [ ] **Step 2: Run** → FAIL. **Step 3: Implement.** **Step 4: Run** — `cargo test -p uc_node --test admin_auth --test reconfig --lib` PASS; clippy clean.
...
**Files:**
- Modify: `uc_node/src/config_file.rs` (`CryptoSection` + `enabled`; new `AdminSection`, `AdminKeyEntry`; `ConfigError::{CryptoChoiceRequired, AdminChoiceRequired, AdminKeys{detail}}`; `StartupOptions.admin: AdminSection`; `load_from_path`), `uc_node/src/bin/uc2-node.rs` (build `AdminPolicy` from `opts.admin` — load key files via `AdminKey::load`, refusal → exit 2 with `uc2-node: admin key …` — then `Node::start_with(cfg, StartOpts { socket: None, admin })`; print `uc2-node: WARNING: [admin] auth = "none" — anyone who can write the instance directory can change membership` at boot when `none`)
...
- Test: `uc_node/src/config_file.rs` tests; `uc_node/tests/daemon_refusals.rs` (new, or extend the existing bin-smoke test if one exists — grep `CARGO_BIN_EXE_uc2-node`)
...
- [ ] **Steps 1–5** as the pattern: failing tests → implement → `cargo test -p uc_node --lib --test daemon_refusals` + clippy → commit `git commit -am "feat(config): [crypto].enabled and [admin] are explicit choices; the daemon builds the admin policy from [admin]"`. Also run `python3 -m py_compile bench-infra/scripts/m9_fleet_gate.py`.
...
**Files:**
- Modify: `uc_ctl/src/main.rs`, `uc_ctl/Cargo.toml` (+ `uc_crypto` path dep)
- Create: `uc_ctl/tests/admin_auth_bin.rs`
...
- [ ] **Step 1: Write the failing test** — `uc_ctl/tests/admin_auth_bin.rs`: start a 1-node cluster IN-PROCESS via `uc_node::Node::start_with(cfg, StartOpts { admin: AdminPolicy::Hmac { keys: [AdminKey "ops-test" from a generated file], ttl: 30 s } })` (uc2ctl already depends on uc_node); spawn `CARGO_BIN_EXE_uc2ctl`:
...
- [ ] **Steps 2–5**: → FAIL → implement → `cargo test -p uc_ctl` PASS (+ `-p uc_node --test reconfig`), clippy → commit `git commit -am "feat(uc2ctl): --admin-key signing, gen-admin-key, audit; reason strings 20-24"`.
