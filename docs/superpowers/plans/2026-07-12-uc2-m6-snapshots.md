...
**Tech Stack:** Rust workspace (edition 2024), `uc_protocol` v2 (no_std), `uc_log`/`uc_net`/`uc_consensus`/`uc_node`/`uc_sim`/`uc_service`/`uc_client`, `uc_journal` (in-tree: `purge_before`, `StableValue`, `TailReader`), sibling `ultima-db` (`persistence` feature: `snapshot_stream`/`install_snapshot_stream`), `uc_lincheck` (checker unchanged — again).
...
- **clippy `--workspace -- -D warnings` + `cargo clippy -p uc_consensus -p uc_sim -p uc_node -p uc_net -p uc_log -p uc_service -p uc_client --all-targets -- -D warnings` stay clean.** Denied lints history: `manual_is_multiple_of`, `int_plus_one`, `collapsible_if` (let-chains).
...
uc_log/src/archive.rs               MOD  first_base tracking, purge_below(pos), gap-aware replay errors
uc_journal/src/journal/tail_reader.rs MOD  scan_from(start_meta) segment skip + first_base()
uc_service/src/traits.rs            MOD  SnapshotStateMachine trait + SnapshotError
uc_service/src/snapshots.rs         NEW  snapshot dir layout, file naming, temp+rename, retention, discovery
uc_service/src/builder_agent.rs     NEW  snapshot builder thread (policy, freeze/stream, cnc publish)
uc_service/src/ultima_db.rs         NEW  StoreStateMachine v2 adapter (feature "ultima_db")
uc_service/src/replay.rs            MOD  gap guard + snapshot-install reconstruction path
uc_service/src/apply.rs             MOD  install-inside-apply-thread wiring
uc_service/src/config.rs            MOD  SnapshotPolicy knobs, ServiceError::SnapshotRequired
uc_protocol/src/v2/cnc.rs            MOD  new slots in 1152.. band (snapshot marker, per-follower obs)
uc_log/src/cnc.rs                   MOD  accessor structs for the new slots
uc_log/src/state.rs                 MOD  NodeState gains snapshot marker StableValue
uc_protocol/src/v2/datagram.rs       MOD  kinds 12-15 SNAP_* + bodies + pins
uc_net/src/sender.rs                MOD  snapshot session state + duty-cycle budget arbitration
uc_net/src/receiver.rs              MOD  snapshot chunk intake + SNAP_NAK + completion handoff
uc_net/src/flow.rs                  MOD  voting-only construction (explicit voter list + cluster_size)
uc_consensus/src/election.rs        MOD  NoCommonPrefix → Action::WipeAndRejoin; learner-aware helpers
uc_node/src/node.rs                 MOD  marker sampling/persist, purge driver, ArchiveCmd enum,
                                          learner config/wiring, snapshot-session orchestration,
                                          per-follower obs publication, wipe-and-rejoin exec
uc_sim/src/world.rs                 MOD  purge + wipe-and-rejoin modeled (oracle unchanged)
uc_sim/tests/scenarios.rs           MOD  no-common-prefix + purge pins
uc_lincheck/src/register.rs          MOD  RegisterSm implements SnapshotStateMachine (feature v2)
uc_node/tests/purge_safety.rs       NEW  purge + reconstruction + lincheck-under-purge
uc_node/tests/learner.rs            NEW  learner join/live/isolation scenarios
uc_node/examples/m6_gate.rs         NEW  learner-join-under-load + purge-safety gate roles
...
**Files:**
- Modify: `uc_log/src/archive.rs`
- Modify: `uc_journal/src/journal/tail_reader.rs`
- Modify: `uc_service/src/replay.rs` (use `scan_from`; the gap GUARD itself is Task 5)
...
```rust
// uc_journal/src/journal/tail_reader.rs
impl TailReader {
...
- [ ] **Step 1: failing archive tests** (`uc_log/src/archive.rs` tests — reuse the existing `archive_with_one_block`-style helpers, extend to multi-block):
...
- [ ] **Step 2: run** `cargo test -p uc_log purge_below` → FAIL (methods missing).
...
- [ ] **Step 6:** switch `uc_service::replay::replay_into` to `scan_from(start_pos, ..)` — no behavior change; existing reconstruction tests stay green and are the regression net.
...
- [ ] **Step 8: verify** — `cargo test -p uc_journal -p uc_log -p uc_service`, both clippy gates.
- [ ] **Step 9: commit** — `git add uc_log/src/archive.rs uc_journal/src/journal/tail_reader.rs uc_service/src/replay.rs && git commit -m "feat(uc2): archive purge_below + first_base tracking; TailReader scan_from — PositionPurged becomes real with correct bounds (M6 floor primitives)"`
...
**Files:**
- Modify: `uc_service/src/traits.rs`, `uc_service/src/config.rs` (`SnapshotError` re-export shape), `uc_service/Cargo.toml` (feature `ultima_db`, optional dep `ultima-db` workspace)
- Create: `uc_service/src/ultima_db.rs`
- Modify (SIBLING REPO — shared checkout, additive commit only): `/home/claude/ultima/ultima_db/src/snapshot_stream/install.rs`
- Modify: `uc_lincheck/src/register.rs` (feature `v2`: RegisterSm implements the capability)
- Tests: inline + `uc_service/tests/snapshot_roundtrip.rs`

**Interfaces:**
- Produces (`uc_service::traits`):
...
- Produces (`uc_lincheck`, feature `v2`): `impl SnapshotStateMachine for RegisterSm` — `SnapshotHandle = Vec<u8>` (bincode of `(value, last_applied)`), freeze/stream/install trivial. This is what lets the L3 harness drive the REAL purge path.
...
- [ ] **Step 2: failing adapter roundtrip** (`uc_service/tests/snapshot_roundtrip.rs`):
...
- [ ] **Step 3: run** → FAIL (trait/adapter missing); implement the trait, the adapter (feature-gated), and the RegisterSm impl + its own small roundtrip unit in uc_lincheck.
...
- [ ] **Step 5: verify** — `cargo test -p uc_service -p uc_lincheck --features v2` (lincheck) + sibling `cargo test -p ultima-db`, both clippy gates (workspace + all-targets set).
- [ ] **Step 6: commit** (ultima_cluster side) — `git add uc_service/src/traits.rs uc_service/src/ultima_db.rs uc_service/src/lib.rs uc_service/src/config.rs uc_service/Cargo.toml uc_lincheck/src/register.rs uc_service/tests/snapshot_roundtrip.rs Cargo.lock && git commit -m "feat(uc_service): SnapshotStateMachine capability + ultima_db StoreStateMachine adapter (position-as-version, v1 u64-return rule)"`

### Task 3: Service-side snapshot builder — files, policy, cnc publish

**Files:**
- Create: `uc_service/src/snapshots.rs`, `uc_service/src/builder_agent.rs`
- Modify: `uc_service/src/lib.rs` (builder spawn for snapshot-capable SMs), `uc_service/src/config.rs` (`SnapshotPolicy`), `uc_service/src/apply.rs` (freeze hook on the apply thread)
- Modify: `uc_protocol/src/v2/cnc.rs` + `uc_log/src/cnc.rs` (ONE new slot now; the rest of the 1152.. band is Task 9)
- Tests: `uc_service/tests/snapshot_build.rs` + inline
...
- [ ] **Step 1: failing e2e build test** (`uc_service/tests/snapshot_build.rs`, single node + RegisterSm-with-snapshots, the M5 test-harness helpers):
...
- [ ] **Step 4: verify** — `cargo test -p uc_service -p uc_log -p uc_protocol`, uc_node regression, both clippy gates.
- [ ] **Step 5: commit** — `git add uc_service/src/snapshots.rs uc_service/src/builder_agent.rs uc_service/src/lib.rs uc_service/src/config.rs uc_service/src/apply.rs uc_protocol/src/v2/cnc.rs uc_log/src/cnc.rs uc_service/tests/snapshot_build.rs && git commit -m "feat(uc_service): snapshot builder — freeze on apply thread, stream off-lock, atomic position-tagged files, cnc marker"`

### Task 4: Node marker persistence + purge driver

**Files:**
- Modify: `uc_log/src/state.rs` (NodeState + `snapshot.state`), `uc_node/src/node.rs` (sampling, persist, mirror, purge command), `uc_node/src/lib.rs` (config re-export)
- Tests: `uc_node/tests/purge_safety.rs` (first scenarios) + inline
...
- [ ] **Step 1: failing node test** (`uc_node/tests/purge_safety.rs`):
...
- [ ] **Step 4: verify** — `cargo test -p uc_node -p uc_log`, failover ×2 (regression — purge default-off must change nothing), both clippy gates.
- [ ] **Step 5: commit** — `git add uc_log/src/state.rs uc_node/src/node.rs uc_node/src/lib.rs uc_node/tests/purge_safety.rs && git commit -m "feat(uc_node): durable snapshot floor (increase-only) + policy-gated purge driver via ArchiveCmd (default OFF)"`
...
**Files:**
- Modify: `uc_service/src/replay.rs` (gap guard + install path), `uc_service/src/apply.rs` (wiring), `uc_service/src/config.rs` (`ServiceError::SnapshotRequired`), `uc_service/src/attach.rs` (doc: drift check unchanged — durable is still the right bound)
- Tests: `uc_service/tests/reconstruction.rs` (extend)
...
- [ ] **Step 1: failing test** (`uc_service/tests/reconstruction.rs`):
...
- [ ] **Step 4: verify** — `cargo test -p uc_service -p uc_node`, both clippy gates.
- [ ] **Step 5: commit** — `git add uc_service/src/replay.rs uc_service/src/apply.rs uc_service/src/config.rs uc_service/src/attach.rs uc_service/tests/reconstruction.rs && git commit -m "feat(uc_service): below-floor reconstruction — gap guard, snapshot install on the apply thread, tail replay; SnapshotRequired fail-stop (kills the silent-gap class)"`
...
**Files:**
- Modify: `uc_protocol/src/v2/datagram.rs` (kinds + bodies + literal pins), `uc_net/src/sender.rs`, `uc_net/src/receiver.rs`, `uc_node/src/node.rs` (orchestration + snapshot-file resolution)
- Tests: `uc_net/tests/snapshot_session.rs` (loopback pair + fault layer) + inline
...
- [ ] **Step 2: failing session test** (`uc_net/tests/snapshot_session.rs`, loopback sender+receiver harness in the existing `common/mod.rs` style):
...
- [ ] **Step 5: verify** — `cargo test -p uc_protocol -p uc_net -p uc_log -p uc_node`, failover ×2, both clippy gates.
- [ ] **Step 6: commit** — `git add uc_protocol/src/v2/datagram.rs uc_net/src/sender.rs uc_net/src/receiver.rs uc_net/tests/snapshot_session.rs uc_log/src/archive.rs uc_protocol/src/v2/cnc.rs uc_log/src/cnc.rs uc_node/src/node.rs && git commit -m "feat(uc_net): snapshot sessions — kinds 12-15, budget-bounded unicast file ship, NAK-repaired, below-floor NAKs upgrade instead of dropping"`

### Task 7: Learner role — fan-out yes, safety quantities no

**Files:**
- Modify: `uc_node/src/node.rs` (config, wiring, probe targeting), `uc_net/src/flow.rs` (voting-only construction), `uc_net/src/sender.rs` (constructor takes voters+learners), `uc_consensus/src/election.rs` (only if a helper is needed — members stays voters-only BY CONSTRUCTION; document)
- Tests: `uc_node/tests/learner.rs` + flow/sender units
...
- [ ] **Step 3: failing cluster test** (`uc_node/tests/learner.rs`, failover-harness style — 3 voters + 1 learner on loopback):
...
- [ ] **Step 6: commit** — `git add uc_node/src/node.rs uc_net/src/flow.rs uc_net/src/sender.rs uc_consensus/src/election.rs uc_node/tests/learner.rs && git commit -m "feat(uc2): learner role — replicated-to, never counted (fan-out yes; vote/quorum/flow/read-quorum no)"`
...
**Files:**
- Modify: `uc_consensus/src/election.rs` (+ sim plumbing in `uc_sim/src/world.rs`, `uc_sim/tests/scenarios.rs`), `uc_node/src/node.rs`
- Tests: `uc_node/tests/learner.rs` (join scenario), `uc_node/tests/purge_safety.rs` (wipe scenario), sim pins
...
- [ ] **Step 1: failing sim pins** (`uc_sim/tests/scenarios.rs`): (a) a no-common-prefix world (follower crashed pre-gossip with divergent-term-only data, leader far ahead — build on the crash_on_next_truncate-style scripting) reaches convergence with `wipes == 1` and the oracle green across 200 seeds; (b) the counterfactual — wipe disabled (old Fatal) — the same world FAIL-STOPs (documents what changed). Mechanical epoch/action plumbing for the sim mirrors Task 1 of M5.
...
- Prefill decision pin (Decision #6): `uc_node/tests/smoke.rs` addition — restart a node, then a NAK below its boot frontier is served from the journal (assert `replay_datagrams > 0`, receiver converges); plus the doc note in the runbook ("prefill rejected — evidence").
...
- [ ] **Step 4: verify** — full uc2 suite + loom (`RUSTFLAGS="--cfg loom" cargo test -p uc_log --test loom_frame --release`) + failover ×2 + both clippy gates.
- [ ] **Step 5: commit** — `git add uc_protocol/src/v2/cnc.rs uc_log/src/cnc.rs uc_log/src/archive.rs uc_node/src/node.rs uc_node/tests/smoke.rs uc_net/src/sender.rs uc_net/src/receiver.rs docs/ops/uc2-runbook.md && git commit -m "feat(uc2): ops — per-peer cnc observability band, prime-generation straddle recheck (M5 residual closed), runbook; prefill decision closed as rejected-with-evidence"`

### Task 10: M6 gates — L3 under purge, m6_gate binary, docs

**Files:**
- Modify: `uc_node/tests/lin_v2.rs` + `uc_node/tests/lincheck_v2/mod.rs` (purge-churn variant), `uc_node/Cargo.toml`
- Create: `uc_node/examples/m6_gate.rs`, `docs/benchmarks/uc2-m6-gate-2026-07-XX.md`
...
- [ ] **Step 6: commit** — `git add uc_node/tests uc_node/examples/m6_gate.rs uc_node/Cargo.toml docs/benchmarks/ && git commit -m "test(uc2): M6 gates — lincheck under purge/snapshot churn, m6_gate learner-join + purge-cycle roles, gate doc + v1 retirement checklist"`
...
cargo test -p uc_sim --features sim-heavy --release          # fuzz incl. wipe arms
RUSTFLAGS="--cfg loom" cargo test -p uc_log --test loom_frame --release
cargo test -p uc_crashtest --features hard-crash-tests       # regression
LIN_SEED=4359 cargo test -p uc_node --test lin_v2 --release  # both capstones; + seeds 7, 99
cargo test -p uc_node --test learner --test purge_safety --release
(cd /home/claude/ultima/ultima_db && cargo test -p ultima-db snapshot_stream)  # sibling change
cargo clippy --workspace -- -D warnings
cargo clippy -p uc_consensus -p uc_sim -p uc_node -p uc_net -p uc_log -p uc_service -p uc_client --all-targets -- -D warnings
cargo run -p uc_node --example m6_gate --release -- all --secs 30   # smoke, honest verdict
```
...
- `uc_client`/`uc_service` dep slimming; PROT_READ service mapping; PSK-MAC slot → v2.x.
