...
7. **L3 harness, ported not rebuilt.** `uc_lincheck` WGL capstone (in-memory RegisterSm) under churn + leader kills over the v2 stack; `lin_partition`'s four scenarios re-driven through the M4 partition handles; `uc-crashtest` SIGKILL (service and node) mid-load on v2 reference bins (`kv_node`/`kv_service`/`kv_client` examples get v2 backends). **This carried harness is the single biggest de-risker in the whole v2 program** (spec §8).
