...
/// `uc2_node/tests/obs_http.rs`'s `get` helper, copied (examples can't
/// import test code).
...
/// `synthetic_sources()` fixture in `uc2_node/src/obs/metrics.rs` — copied,
...
fn scenario_service_wedged(scratch_root: &Path) -> (SeriesFile, Disclosure) {
    ...
    for _ in 0..12 {
        sf.record_round(
            "n0",
            &scrape(addr),
            &["uc2_service_heartbeat_age_seconds", "uc2_node_heartbeat_age_seconds"],
        );
        thread::sleep(Duration::from_secs(1));
    }
    nodes[0].stop();
...
// partition / await_single_leader) and `uc2_node/tests/obs_http.rs`
