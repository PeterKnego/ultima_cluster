...
//! [`uc_remote::frame`], answers with real frames, and records what it saw, so
//! the client tests assert on wire behaviour rather than on call expectations.
//! No cluster, no `uc_gateway` — just enough edge to exercise credits,
