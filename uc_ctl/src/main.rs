...
use uc_crypto::admin::generate_key_file;
use uc_ctl::{
    CommonArgs, key_name_from_path, open, reason_str, schedule, settings, signed_admin_request,
};
...
#[derive(Subcommand)]
enum SettingsCmd {
    /// Parse, stage, sign and apply the replicated settings record
    /// (`ADMIN_OP_SETTINGS_APPLY`, wire op 7) — see `uc_ctl::settings`'s
    /// module doc for the TOML shape.
    Apply(SettingsApplyArgs),
    /// Print the newest ADOPTED settings (durable node state, not the
    /// staged file).
...
struct SettingsApplyArgs {
    #[command(flatten)]
    common: CommonArgs,
    /// Path to the settings TOML file (cluster-FSM spec §6).
    file: PathBuf,
}
...
fn run_status(a: &StatusArgs) -> anyhow::Result<()> {
    ...
    // this instance directory's newest cluster artifact — `none` when there
    // is no artifact yet (position 0 never occurs for a real committed
    // frame, so it is an honest "nothing committed" sentinel here, task 8's
    // deferred finding from task 5: printing a bare `0` reads as "position
    // zero", which is indistinguishable from "healthy, at the origin"). Read
    // directly off disk (like `schedule show`), not through the cnc page:
    // the table isn't on the cnc page at all.
