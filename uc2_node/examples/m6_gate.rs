...
//!
//! ```text
//! cargo run -p uc2_node --release --example m6_gate -- all      --secs S   # local smoke, NOT the gate
//! cargo run -p uc2_node --release --example m6_gate -- node     --id N --bind A --members … --instance-dir D [--learners …]
//! cargo run -p uc2_node --release --example m6_gate -- service  --instance-dir D
//! cargo run -p uc2_node --release --example m6_gate -- learner  --id N --bind A --members … --instance-dir D
//! ```
//!
...
fn spawn_service(dir: &std::path::Path) -> uc2_service::Service<RegSm> {
...
fn run_all(a: AllArgs) -> anyhow::Result<()> {
    ...
    let mut services: Vec<Option<uc2_service::Service<RegSm>>> = Vec::with_capacity(NV);
...
fn scenario_purge_cycle(
    nodes: &[Node],
    services: &mut [Option<uc2_service::Service<RegSm>>],
    dirs: &Arc<Vec<PathBuf>>,
    load: &Load,
    cycles: u32,
) -> Verdict {
