...
| **Snapshot-session framing** (`SNAP_BEGIN` 12, `SNAP_CHUNK` 13, `SNAP_NAK` 14, `SNAP_DONE` 15) | `datagram.rs: read_snap_begin_body` (`SNAP_BEGIN_FIXED_LEN = 34` + a variable config tail; wire 0.6.0/M14c adds `layout`, `service_id`, `services_declared`), `read_snap_nak_body` | any peer | crypto ON: yes — **all** SNAP kinds are sealed since M8 T17 | fixed prefix + a bounded config tail; total readers (any `layout` value decodes; the receiver decides). Two named refusals on the receiving node bound a mixed-version or mis-declared peer: `peer wire 0.5.0` (a body whose `layout` discriminator the receiver does not speak) and `declared-set mismatch` (a session whose `services_declared` doesn't match the receiver's own set) | `uc_protocol_datagram` | A forged `SNAP_BEGIN` would carry attacker-chosen application state *and* membership into a joining node; the M8 fall-through that closes it is pinned by `an_unsealed_snap_begin_is_refused_now_that_t17_landed`. Since 0.6.0 a session carries one artifact per declared id, so a forged `SNAP_BEGIN` (crypto off) can populate **N** `snapshots/<id>/` directories on the joiner, not one; the id is bounded (`< 64`, and must be in the receiver's own declared set — `receiver.rs:1653`, `:1837`) so it selects among ≤ 8 fixed directories and never a path. |
