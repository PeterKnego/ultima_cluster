...
| f | Correctness capstones on the new client: `remote_lin` (envelope on/off), `uc_gateway` tests, `client_fake_edge` suite ported | green | **PASS** (local, 2026-08-25): `uc_remote` all bins + `uc_gateway` all bins (incl. grant budget) + `remote_lin` 2/0 |
...
cargo test -p uc_crashtest --features hard-crash-tests --test remote_lin  # row f: the capstone, envelope on/off
```
