...
//! well-known shmem files `uc_node` creates (`cnc2.dat`, `ingress.ring`,
...
/// Same per-record ceiling `uc_node::create_rings` gives every ring.
const MAX_MSG: u32 = 64 << 10;
/// Same default the real node publishes (`uc_node`'s `default_admission_bytes`).
