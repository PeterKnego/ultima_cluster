...
//! converges."*
//!
//! Real multi-process (3 `uc_crashtest-node`/`uc_crashtest-service` pairs,
...
//! (`uc_node::backup::backup_instance`, in-process — this test crate links
//! `uc_node` directly, no shelling out to `uc2ctl`), that follower's
...
//!
//! `uc_lincheck::register::RegisterSm` is a single-value register: an
...
/// same choreography `uc_node/tests/reconfig.rs`'s `admin_request` uses):
...
/// `uc_node::recovery::force_single_member` (in-process — this test crate
/// links `uc_node` directly, same as the backup/restore calls above), comes
