...
/- UC v2.1 (M7) reconfig commit plane — ABSTRACT-QUORUM model for the SMT/inductive
   ...
   * THE OPEN-CLAUSE TRUTH RULE (gate 1c, binding): a clause with open CTIs is a LIVE
   ...
   SESSION-3 (bar 3, part 3) STATE — see proofs-veil/spike-ledger.md §SESSION 8:
     * `#check_action propose` IS run (bottom of file). **RUN 12: 409 OK / 8 CTI —
...
-- veil.smt.timeout N in #check_action propose` does NOT propagate to the solver calls (run 19
...
veil module UcReconfigCommitSMTHSpropose
...
-- ---------- unverified; expect this set to change under #check_action propose) ----------
...
-- ---- SESSION 7 (bar 3, part 6): THE HOLDER-SUPPLY CHAIN, item 45's T23/T24 ----
-- MEASUREMENT-ONLY FILE. These two clauses are the session-6 map's chain, stated
-- verbatim from ledger item 45. The session-7 truth analysis says T24 is FALSE in a
-- REACHABLE state (stale leader at genesis proposes after a genesis commit); this run
-- exists to obtain the CHECKER's CTI for that written argument. Nothing here lands in
-- the model unless the checker says otherwise.
invariant [proposal_above_commit_after_e]
  (committed ∧ hasProposal I ∧ cfgLt commitCfgid (proposedC I)) → propAfterE I
invariant [above_commit_cfg_implies_holder]
  (committed ∧ cfgLt commitCfgid (cfgOf N)) → holdsE N
