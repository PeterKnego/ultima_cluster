...
- [ ] **Step 1**: Read the Rust ground truth: `uc_log`'s appender/receiver contiguity (module docs suffice), `uc_consensus/src/election.rs` `become_leader` (post-prune form) + `DataTermObserved` arm, `uc_consensus/src/reconcile.rs` (the kernel you re-use via `Uc2Model.reconcile`), and node.rs's reconcile-then-truncate action flow (grep `Action::Truncate`). Also re-read the spike model + S1 report.
