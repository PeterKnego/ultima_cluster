...
veil module UcReconfigCommitSMTT15Slice
...
-- ---------- SLICE S1 (session 6): certify T15 `role_below_quorum_strict` ----------
-- The model is UNCHANGED (same 35 requires / 11 assumptions, same actions); only the
-- invariant conjunction is cut down to what T15's preservation consumes, plus T15 itself.
-- A slice OK transfers to the full bundle (Inv_full -> Inv_slice only weakens each VC's
-- antecedent); a slice CTI/timeout transfers in neither direction. Ledger item 37.
-- HYPOTHESIS SET (recorded for the transfer audit): role_exclusive, cand_cfg_frozen,
-- eleccfg_not_ahead, reach_bound, reach_mono, cfg_from_genesis, self_vote, grant_state,
-- leader_reach_strict, reach_quorum_below, role_below_quorum_strict.
...
-- T15 — THE TARGET. Truth argument in the ledger (SESSION 11), written before this run.
