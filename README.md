...
Download the signed `v2.7.0` tarball and run its quickstart — no toolchain
needed:

```bash
tar xzf uc2-2.7.0-x86_64-unknown-linux-gnu.tar.gz
uc2-2.7.0-x86_64-unknown-linux-gnu/packaging/quickstart-local.sh
```
...
| *on `main`* (M14) | Several state machines per cluster, fed by one log; submit to any, or fan a query across all | [Design spec](/docs/superpowers/specs/2026-08-21-uc2-multi-service-design.md) — unreleased |
