...
- **With crypto OFF, a forged `PROBE_ACK` can raise the cluster's datagram
  rung, and the damage is PERSISTENT** (`2.12.0`, jumbo-frame MTU discovery) —
  [the explainer § The security posture](/docs/notes/uc2-jumbo-frame-discovery-explained.md#the-security-posture-in-one-paragraph).
  `PROBE`/`PROBE_ACK` are `Scope::Pairwise`, so with `[crypto].enabled = true`
  they are sealed per peer and unforgeable by a network-path adversary. With
  crypto off nothing authenticates the source (the row above), so an injected
  ack claiming the top rung walks the committed ceiling up to a size the paths
  cannot carry — `uc2_send_emsgsize_total` climbs, commits stall, and a
  restarting node refuses to join. This is the **same threat class as forged
  `DATA`**, in a new flavour, with one property the others lack: the rung is
  **monotone** and persists in the cluster artifact, so the damage survives
  every restart and there is no downgrade path. The remedy is the one this
  model already names — `[crypto].enabled = true` on any network where an
  attacker can inject datagrams.
