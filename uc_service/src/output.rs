...
//!   12 / `uc_node::node`), `output_progress` can legitimately lag the
...
//! walking the archived journal via [`uc_journal::TailReader`] — the same
    ...
    loop {
        ...
        match result {
            ...
            Err(OutputError::Permanent(msg)) => {
                eprintln!(
                    "uc_service: on_committed PERMANENT failure at position {pos}: {msg} \
