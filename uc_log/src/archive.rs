...
// The replay-session path (uc_net, M4) shares the archive's journal handle
...
mod tests {
    ...
    /// Same discipline as `uc_net::sender::tests::crypto_transport`.
    ...
    /// lives in `uc_node` (`ArchiveCmd::Collapse` / `Consensus::on_collapsed`),
    ...
    ///
    /// `Action::BecomeLeader` (uc_node `node.rs`) collapses the volatile tail
    ...
    /// contract `uc_node`'s `ArchiveCmd::Collapse` exists to hold.
    #[test]
    #[cfg_attr(miri, ignore)] // real journal files + fsync
