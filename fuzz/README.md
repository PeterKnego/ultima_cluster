...
| `uc_protocol_cluster_frame` | `uc_protocol::v2::frame::read_cluster_prefix` plus the kind-dispatched payload decoders (`config::decode_config`, `schedule::decode_schedule_table`, `settings::decode_settings`) — the `CLUSTER` body every node decodes off the log (spec §4.3). |
