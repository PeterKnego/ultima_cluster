...
impl ServiceStatusLine {
    ...
    /// Plan B2: the version the artifact at `upgrade_origin` was BUILT by —
    /// the pin's `from`. Fourth word under the same seqlock; see
    /// [`ServiceStatusLine::pin`].
    pub fn pinned_from(&self) -> u32 {
        self.pinned_from.load(Ordering::Acquire) as u32
    }
    /// The seqlock commit word guarding the pin words (cnc 3.3, slot +32).
    ...
    /// writer that has stored the new `to`/`from` but not yet the new
    /// `origin` leaves the origin stable across the reads, so a naive
    /// reader returns `(origin_old, from_new, to_new)` — a triple that
    /// never existed. The words therefore ride a real seqlock, `pin_seq`.
    ///
    /// Read `pin_seq`, all three words, then `pin_seq` again; accept only if
    /// the first read was EVEN (no store in flight) and the two reads
    /// match (no store completed in between). The orderings are the file's
    /// seqlock discipline — the commit word is `Acquire`-loaded, the
    /// writer's stores are `Release` — and no extra fence is needed here
    /// because, unlike `read_admin_req`'s plain byte fields, the data words
    /// are themselves atomics: if a data-word load observes a value a
    /// `store_pin` released, that store synchronizes-with this load, so
    ...
    /// thus brackets a window in which no `store_pin` touched any data word.
    ///
    /// Bounded at 64 spins rather than looping forever, and on exhaustion
    /// returns [`PinRead::Contended`] — NOT a best-effort or fabricated
    /// triple. The writer is the `uc2-cluster` agent republishing the same
    /// pin idempotently on every view publish, so 64 consecutive collisions
    /// mean something pathological; under that, "could not read this row's
    /// pin right now" is an honest, NAMED answer and the next call
    /// converges, while a fabricated triple is not — plan B2's attach would
    /// install the wrong artifact, or wrongly refuse, from one.
    ...
    /// Publish the triple under the `pin_seq` seqlock: bump to ODD, store
    ...
    /// left undone. Argument order matches `store_pin`'s trailing pair
    /// (`from`, `to`). `pub` + `#[doc(hidden)]` (not `#[cfg(test)]`): plan
    /// B2's task 4 integration test lives in another crate and needs to
    /// hold the seqlock open from outside `uc_log`.
    #[doc(hidden)]
    pub fn store_pin_begin_for_test(&self, from: u32, to: u32) {
        self.pin_seq.fetch_add(1, Ordering::Release);
        self.pinned_version.store(to as u64, Ordering::Release);
        self.pinned_from.store(from as u64, Ordering::Release);
    }
    /// The second half of [`ServiceStatusLine::store_pin`] — the origin
    /// store and the closing EVEN bump. Pairs with
    /// [`ServiceStatusLine::store_pin_begin_for_test`].
    #[doc(hidden)]
    pub fn store_pin_finish_for_test(&self, origin: u64) {
        self.upgrade_origin.store(origin, Ordering::Release);
        self.pin_seq.fetch_add(1, Ordering::Release);
    }
}
...
mod tests {
    ...
    /// `to`/`from` but not yet the NEW origin leaves the origin stable, so
    /// any amount of re-reading it yields `(origin_old, from_new, to_new)` —
    /// a triple that was never stored. With the seq word odd, `pin()`
    /// refuses to answer at all, naming the contention rather than
    /// fabricating one.
    #[test]
    fn pin_is_contended_while_a_store_is_in_flight() {
        ...
        assert_eq!(
            s.pin(),
            PinRead::Contended,
            "mid-store: named, never a fabricated triple"
        );
