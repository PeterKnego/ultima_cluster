...
//! other's code — from `uc2_node/examples/m10_alerts.rs`, itself copied from
//! `uc2_node/tests/failover.rs`. `/readyz` never returns 200 without an
//! attached service (`service_heartbeat_ns` is only ever stamped by
//! `uc2_service`'s apply agent, so a service-less node reads as
...
use uc2_node::obs::metrics::CONTRACT_SERIES;
...
/// ever stamped by `uc2_service`'s apply agent (`uc2_service/src/apply.rs`).
...
// `uc2_node/examples/m10_alerts.rs`, itself derived from
// `uc2_node/tests/failover.rs`.
...
// ------------------------------------------------------------- HTTP scrape
//
// Copied (not imported) from `uc2_node/examples/m10_alerts.rs`'s minimal
// blocking HTTP/1.1 GET client, same shape as `uc2_node/tests/obs_http.rs`.
