...
//! Test-only fixtures for standing up REAL `uc2_crypto` state — key files,
//! allowlists, completed Noise-IK handshakes, delivered group keys — using
//! only `uc2_crypto`'s public API.
//!
...
use uc2_crypto::{HandshakeAction, NodeId, SharedTransport};
...
pub fn scratch_dir(area: &str, tag: &str) -> PathBuf {
    ...
    let dir = std::env::var("CARGO_TARGET_TMPDIR")
        .map(PathBuf::from)
        .unwrap_or_else(|_| PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../target/uc2_net_tests"))
        .join(area)
        .join(format!("{tag}-{seq}"));
...
/// `uc2_crypto::identity::Identity::load` insists on.
...
/// throwaway `Identity` — `uc2_net` has no X25519 dependency of its own (nor
...
/// `uc2_crypto::identity`'s allowlist parser (which uses the `base64` crate's
/// `STANDARD` engine) — hand-rolled rather than adding a `base64`
/// dev-dependency to `uc2_net` for one fixture's allowlist text.
