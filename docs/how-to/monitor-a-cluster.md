...
is failing. After 2 s with the gate shut (well inside a service's 10 s
`boot_wait`) the node says which clause is holding it, once, as
`services_declared_withheld` (below).
