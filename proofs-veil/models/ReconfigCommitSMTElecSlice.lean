...
set_option veil.smt.timeout 900

veil module UcReconfigCommitSMTElecSlice
...
-- ---------- ELECTION-SAFETY SLICE (session 5) ----------
-- Only `election_safety` + the clauses T8's chain argument consumes. A ✅ here TRANSFERS to
-- the full bundle: `#check_invariants` proves `Inv_slice(s) ∧ action → clause(s')`, and the
-- full bundle's `Inv_full` IMPLIES `Inv_slice`, so the full-bundle VC is implied. The slice
-- is a cost device, not a weakening.
...
-- RUN 22: the same slice with a 900 s per-VC budget. Run 21 left exactly one ⏱️
-- (`election_safety` at `becomeLeader`); the slice is small enough that a big budget is
-- affordable here even though it is not affordable on the 41-clause bundle.
#check_invariants
