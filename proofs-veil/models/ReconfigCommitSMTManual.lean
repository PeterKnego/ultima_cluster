...
veil module UcReconfigCommitSMTManual
...
-- ------- SLICE S4 (session 6): election_safety over the LEAN T17 chain -------
-- Run 21/23's election slice carried 17 clauses and left `election_safety`@becomeLeader at
-- timeout even with 900 s per VC. This slice keeps ONLY what T8's chain consumes once T17
-- has pre-composed the `reach_quorum_below` -> adjacency step, i.e. it removes the seven
-- clauses that were there solely to preserve T15/T17 (which are certified in runs 26/27).
-- Clauses other than `election_safety` may show slice artifacts; only its verdict is read.
...
theorem becomeLeader_election_safety (ρ : Type) (σ : Type) (node : Type) [node_dec_eq : DecidableEq.{1} node]
    ...
    [becomeLeader_dec_0 :
      delta% @UcReconfigCommitSMTManual.becomeLeader._veil_dec_type_0 node quorum χ term cfgid χ_rep] :
    ∀ (i : node) (q : quorum),
  ...
  rcases hinv with ⟨hes, hgs, hgu, hsv, hre, hccf, hena, hew, hegcr, hgrc, hrbmq⟩
