...
//! shared-memory IPC. The WGL checker / history / model in `uc_lincheck` are
...
// instead of `Disabled`. Mirrors `uc_node/tests/crypto_cluster.rs`'s own
...
/// a LOCAL, optimistic accept (confirmed against `uc_node/tests/reconfig.rs`'s
    ...
    LinClusterV2<SM, SM1>
{
    ...
    pub fn start_cfg(root: &Path, n: usize, faults: FaultConfig, ccfg: ClusterCfg) -> Self {
        ...
        // `uc_node/tests/reconfig.rs`'s `joining_node_boots_from_stale_seed`),
    ...
    /// bin (mirrors `uc_node/tests/reconfig.rs`'s `open_cnc`).
    ...
    /// tombstone rule: a removed id can never rejoin, see `uc_consensus`'s
...
// (`uc_service::StateMachine::apply(&mut self.0, ..)`), never `self.0.apply(..)`:
...
impl<SM: uc_service::SnapshotStateMachine + uc_service::StateMachine, const MICROS: u64>
    uc_service::SnapshotStateMachine for Slow<SM, MICROS>
{
    type SnapshotHandle = SM::SnapshotHandle;
    fn freeze(&self) -> Result<(SM::SnapshotHandle, u64), uc_service::SnapshotError> {
        self.0.freeze()
    }
    fn stream_snapshot(
...
impl uc_service::SnapshotStateMachine for Corrupt<RegisterSm> {
...
/// journal instead" (`uc_service/src/replay.rs:73-78`'s gap guard: install
...
/// (`uc_node/tests/lin_v2.rs`) is the ONLY test in this binary that wraps an
...
impl uc_service::SnapshotStateMachine for InstallCounting {
    type SnapshotHandle = <RegisterSm as uc_service::SnapshotStateMachine>::SnapshotHandle;
    fn freeze(&self) -> Result<(Self::SnapshotHandle, u64), uc_service::SnapshotError> {
        self.0.freeze()
    }
