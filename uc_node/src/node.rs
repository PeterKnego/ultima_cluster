...
pub struct NodeConfig {
    ...
    /// Jumbo spec §6: make discovery a STARTUP GATE. `false` (the default) is
    /// today's behaviour — a cluster starts at the baseline rung and rises
    /// only when every member proves a larger path. `true` holds `can_serve`
    /// false until this node has proven [`JUMBO_MIN_RUNG`] to every configured
    /// member, and fail-stops the consensus agent by name
    /// ([`JUMBO_GATE_WINDOW`]) if it cannot — for an application whose
    /// commands do not fit the standard ceiling, a cluster that cannot carry
    /// them must not start.
    ///
    /// `[crypto]`-shaped per-host config, not a wire flag day: a cluster may
    /// run it on some hosts and not others (each gates only itself).
    pub force_jumbo_frames: bool,
...
// not repeated). `uc_net` carries no logging dependency — the node names it.
...
/// Jumbo spec §6: how long `force_jumbo_frames` holds serving open, waiting
/// for every configured member to prove [`JUMBO_MIN_RUNG`], before the node
/// fail-stops by name. A constant, not a knob (spec §12) — an operator who
/// wants a different answer wants a different flag value.
///
/// The `Joining` gate (spec §5.4) has NO window: it passes on a proven
/// quorum and holds otherwise, however long that takes — see [`JumboGate`]
/// for why a timer would be the wrong tool there.
...
/// pending gate can last a while — silence never refuses, so a node restarted
/// into a degraded cluster holds its `Joining` gate until a quorum of voters
/// proves the rung, which with a quorum down is until one comes back. At
/// 10 Hz that costs nothing and no probe round can be missed: the ladder's
/// fastest cadence is 1 s. Same idiom, and the same table, as
/// `next_mtu_check_ns`.
const JUMBO_GATE_POLL_NS: u64 = 100_000_000;

/// How often a HOLDING `Joining` gate says so (`jumbo_join_gate_holding`,
/// `Warn`). The hold has no timer, so it needs a voice: `jumbo_join_gate_armed`
/// is written once and rotates out of any log, and a held FOLLOWER moves no
/// alerting series except `uc2_jumbo_gate_pending` (which `Uc2JumboGateHeld`
/// watches). The probe ladder's slow cadence, so a hold that outlasts the
/// fast ladder logs once per slow round — never per poll.
const JUMBO_GATE_HOLD_LOG_NS: u64 = 30_000_000_000;
...
///
/// The two gates differ in what they are protecting, and in what ends a
/// hold:
///
...
///   snapshot — and takes precedence over `Forcing` (spec §6): on a cluster
///   that has committed a jumbo rung the join rule below is the one that
///   runs, and it is NOT the force gate's rule. Every rung above the baseline
///   is at or above `JUMBO_MIN_RUNG`, so a peer that proves the join rung has
///   proven the force rung too — but the join gate passes on a QUORUM, so a
///   silent voter outside it, or any learner, may be unproven when a node
///   under `force_jumbo_frames` starts serving. An operator who wants the
///   force gate's every-member answer on such a cluster reads
///   `uc2_probe_min_mtu_bytes`, which still reports the minimum over all.
///
/// The `Joining` gate has three outcomes, checked in this order on every
/// poll:
///
/// 1. It REFUSES a peer whose path is PROVEN narrower than the committed
///    rung — it answered, and its fast ladder is spent, so the larger rungs
///    were actually tried (`ProbeTable::narrow_peers`). No window: the
///    ladder IS the settle. Any such peer refuses, voter or learner, quorum
///    or no quorum — the log's frames must cross every member's path.
/// 2. It PASSES once the VOTERS that have proven the rung form a quorum
///    with this node (`Consensus::jumbo_quorum`), the same condition an
///    append needs to commit. A joining LEARNER has no vote and needs no
///    quorum, so it passes on ONE proven voter (see `jumbo_quorum` for why
///    a majority would be wrong there). A learner peer's proof counts for
///    nothing, as its ack counts for nothing at commit.
/// 3. Otherwise it HOLDS `can_serve` false — on silence (a member that is
///    down, slow, or still replaying a cold start), on a peer still inside
///    its fast ladder (MID-DISCOVERY: `verified == RUNGS[0]` with the jumbo
///    rungs in flight is what healthy discovery looks like), or on an ack a
///    peer has since outlived. Refusing any of these fail-stopped healthy
///    nodes, which under `Restart=on-failure` is a crash loop.
///
/// The hold has NO window, and needs none — but it has a VOICE: every
/// [`JUMBO_GATE_HOLD_LOG_NS`] a holding gate writes `jumbo_join_gate_holding`
/// with the terms it is short of and the members short of the rung, and the
/// `uc2_jumbo_gate_pending` gauge it holds at `1` is what `Uc2JumboGateHeld`
/// alerts on, in any role. Proof is a probe ack over the same UDP plane
/// replication uses, from a voter, and `PROBE`, `APPEND_POSITION` (the
/// durable report commit counts) and `VOTE` are all pairwise-sealed: a node
/// that cannot get a probe ack from a quorum of voters cannot get their
/// commit acks or their votes either, and serving is leader-only in UC, so a
/// timed pass would let it do nothing.
/// The outage that once forced a timer here — one dead voter out of three
/// holding every restarted survivor at `/readyz` 503 until it returned —
/// cannot recur, because the survivors ARE a quorum: a restarted one proves
/// the rung to the other and serves within a probe round. What this rule
/// restores over the timed, unproven pass it replaced (2.12.0 review, the
/// maintainer's call): a previously-proven member whose path degraded while
/// it was down — the likeliest misconfiguration, a shrunk interface MTU —
/// finds every voter answering it at the baseline, spends its ladder, and
/// refuses by name instead of joining and serving.
///
/// `deadline_ns == 0` on `Forcing` means NOT YET ARMED: the gate is installed
/// at construction, before any pass has read a clock, so the deadline is
/// computed on the first pass that evaluates the gate — from that pass's one
/// clock read, not from a second read of its own.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum JumboGate {
    ...
    /// prove it carries it before it serves. It refuses a path PROVEN narrow
    /// (answered, and past its fast ladder) IMMEDIATELY; it passes once a
    /// quorum of voters has proven the rung with it; it holds on anything
    /// less conclusive, for as long as that lasts.
    Joining {
        committed: u32,
    },
    Passed,
...
/// What [`Consensus::jumbo_quorum`] found on one poll of a `Joining` gate:
/// the terms every record about the gate carries.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
struct JumboQuorum {
    ...
    self_vote: usize,
    /// The gate's verdict under the role's rule.
    passes: bool,
}
...
pub struct Node {
    ...
    route_drops: Arc<uc_net::receiver::FollowerStats>,
...
impl Node {
    ...
    pub fn start_with(cfg: NodeConfig, opts: StartOpts) -> io::Result<Node> {
        ...
        // rename; see `uc_service::builder_agent`).
                ...
                match cmd {
                    ...
                    ArchiveCmd::Purge { below } => {
                        ...
                        match archive.purge_below(below) {
                            ...
                            Err(e) => {
                                eprintln!(
                                    "uc_node: archive purge_below({below}) failed: {e} \
        ...
        let mut consensus = Consensus {
            ...
            next_mtu_check_ns: 0,
            // Jumbo spec §6. Armed here and not from the recovered view: a
            // node whose instance dir already holds a jumbo rung arms
            // `Joining` instead, on its first `refresh_from_view` (the live
            // rung starts at the baseline, so any committed rung above it is
            // that pass's raise edge).
            jumbo_gate: if cfg.force_jumbo_frames {
                Some(JumboGate::Forcing { deadline_ns: 0 })
            } else {
                None
            },
            jumbo_check_ns: 0,
    ...
    pub fn crypto_stats(&self) -> &uc_net::receiver::FollowerStats {
        &self.route_drops
    }
    ...
    /// [`uc_net::receiver::NetEvent::kind_idx`] (Report, CommitGossip,
    /// RequestVote, Vote, TermMap, LeaderActivity, ReadProbe, ReadProbeAck,
    /// ConfigProposal, ConfigReply).
    pub fn net_event_drops_by_kind(&self) -> [u64; uc_net::receiver::NET_EVENT_KINDS] {
        let mut out = [0u64; uc_net::receiver::NET_EVENT_KINDS];
    ...
    /// (`uc_log::archive`), so reaching it means the bytes are on disk, not
    /// merely queued.
    ///
...
struct Consensus {
    ...
    /// Jumbo spec §6/§5.4: the startup gate, or `None` when this node holds
    ...
    /// has passed.
    jumbo_check_ns: u64,
    /// The monotonic instant a HOLDING `Joining` gate next writes its
    /// `jumbo_join_gate_holding` record ([`JUMBO_GATE_HOLD_LOG_NS`]). `0` =
    /// armed, first interval not yet started (the deadline is computed on the
    /// first holding poll, from the pass's one clock read). Reset to `0` on
    /// every arm.
    jumbo_hold_log_ns: u64,
    ...
    /// that `Node::can_serve` reports as not serving. `true` whenever no gate
    /// is installed.
    jumbo_gate_open: bool,
...
impl Consensus {
    ...
    fn do_work(&mut self) -> bool {
        ...
        // proven it can carry the cluster's datagram budget must not take
        // commands, fire timers or answer linearizable reads, so step 3's
        // `serving`, step 5's `can_serve_flag`/`publish_status` and the read
        // barrier in `advance_pending_reads` all read this one field (review
        // minor 5: the first and last were missing, so a gate-pending leader
        // still drained the ingress ring and answered reads for a client whose
        // `serving_gate` was false). Costs one `Option` test once the gate has
        // passed, or when none was installed at all; it needs `pass_mono_ns`,
        // which the block above has just set.
        self.jumbo_gate_open = self.check_jumbo_gate();
        // Spec §6, review fix 2: `/readyz` reads this, because the cnc
        // `CAN_SERVE` bit it keys on only refuses a LEADER.
        self.jumbo_gate_pending
            .store(!self.jumbo_gate_open, Ordering::Release);
        ...
        // `jumbo_gate_open` (review minor 5): while a startup gate is pending
        // this node is not serving, so it must not fire timers or drain the
        // ingress ring either — "holds serving" was advisory on a leader
        // without it, because `can_serve_flag` is published in step 5 and
        // nothing here read it.
        let serving =
            self.jumbo_gate_open && self.leader_flag.load(Ordering::Relaxed) && self.sm.can_serve();
        ...
        //
        // Jumbo spec §6/§5.4: and off the startup gate, for the same reason —
        // a node that has not proven it can carry the cluster's datagram
        // budget must not take commands. Evaluated once per pass with the
        // clock read at the top of this cycle, so step 3 and this step cannot
        // disagree about it.
        self.can_serve_flag.store(
            self.jumbo_gate_open && !self.halt_removed && self.sm.can_serve(),
            Ordering::Release,
        );
            ...
            && let Some(act) = queue.pop_front()
        {
            ...
            match act {
                ...
                HandshakeAction::Established {
                    ...
                    // that responsibility INSIDE `uc_crypto` — see
    ...
    /// `service_attached` (`uc_service::attach` bumps the epoch once per
    ...
    /// refusal in a log line. The receiver counts them (it is in `uc_net`,
        ...
        if !self.peer_band_published {
            for (i, (id, role)) in self.peer_band.iter().enumerate() {
                self.cnc
                    .peer_slot(i)
                    .id_and_role
                    .store_release(uc_log::cnc::pack_id_and_role(*id, *role));
            }
    ...
    fn publish_status(&mut self) {
        ...
        if !self.halt_removed {
            ...
            // Jumbo spec §6: masked off the startup gate exactly as
            // `can_serve_flag` is in step 5 (`jumbo_gate_open` is THIS pass's
            // answer — step 5 runs before this), so `/readyz` refuses while a
            // gate is pending instead of advertising a node that
            // `Node::can_serve` already reports as not serving.
            if self.jumbo_gate_open && self.sm.can_serve() {
    ...
    fn refresh_from_view(&mut self) {
        ...
        if rung != self.live_mtu.load(Ordering::Relaxed) {
            ...
            // Jumbo spec §5.4: a rung the CLUSTER has committed and this node
            // has not PROVEN is a join refusal, not a door it may open.
            //
            // Edge-triggered on the raise — which is every way a node can
            // learn one: the recovered artifact (the live rung starts at the
            // baseline, so the first pass sees the raise), the archive walk's
            // replay after a restart, and the artifact a snapshot session
            // installs. Edge-triggering is also what keeps this off a healthy
            // node: an ordinary discovery commit is made only once every
            // member has ADVERTISED the rung, this node included, so its own
            // minimum is already at or above what it is adopting; and a later
            // membership change that leaves a fresh peer unresolved — which
            // does drop `own_min_rung` to 0 — moves no rung and so arms
            // nothing.
            let own = self.probe_table.own_min_rung();
            if rung > MTU_DEFAULT && own < rung as u32 {
                self.jumbo_gate = Some(JumboGate::Joining {
                    committed: rung as u32,
                });
                self.jumbo_check_ns = 0;
                self.jumbo_hold_log_ns = 0;
                crate::obs_event!(
                    Warn,
    ...
    /// Jumbo spec §6/§5.4: may this node serve, given its startup gate?
    ///
    ...
    /// window at all. A `Joining` gate passes once a quorum of voters has
    /// proven the rung with this node, and holds otherwise (see
    /// [`JumboGate`]). The agent's failed flag is what `uc2-node`'s monitor
    /// loop turns into `agent_failstopped` and exit 1, exactly as the ring and
    ...
    fn check_jumbo_gate(&mut self) -> bool {
        ...
        match gate {
            ...
            // pass — correctly: it has no peer whose path could be narrow.
            JumboGate::Forcing { deadline_ns } => {
                ...
                match deadline_ns {
                    0 => {
                        self.jumbo_gate = Some(JumboGate::Forcing {
                            deadline_ns: self.pass_mono_ns + JUMBO_GATE_WINDOW.as_nanos() as u64,
                        });
                    }
                    d if self.pass_mono_ns >= d => {
                        self.jumbo_force_fail_stop(JUMBO_GATE_WINDOW);
                    }
                    _ => {}
                }
                false
            }
            // Spec §5.4, rebuilt around a quorum (see `JumboGate`). Three
            // outcomes, in this order:
            //
            // 1. REFUSE a peer whose path is PROVEN narrower than the
            //    committed rung — it answered, and its fast ladder is spent,
            //    so the jumbo rungs were tried and did not land
            //    (`ProbeTable::narrow_peers`). No window: the ladder IS the
            //    settle. Checked FIRST, so a proven quorum never excuses a
            //    member this node provably cannot carry the rung to.
            // 2. PASS once the voters that have proven the rung form a quorum
            //    with this node (a learner: one proven voter). A `Forcing`
            //    gate this one replaced is satisfied for every peer THIS
            //    rule counted — the rest are the residual `JumboGate` names.
            // 3. HOLD otherwise — silence, mid-ladder, an outlived ack. No
            //    timer: with no proven quorum there is no quorum to serve
            //    with, and the dead-host outage a timer once guarded against
            //    cannot recur, because the surviving voters are the quorum.
            JumboGate::Joining { committed } => {
                let narrow = self.jumbo_narrow_peers(committed);
                if !narrow.is_empty() {
                    self.jumbo_join_fail_stop(committed, &narrow);
                }
                let q = self.jumbo_quorum(committed);
                if q.passes {
                    self.pass_join_gate(committed, &q);
                    return true;
                }
                // Holding. A hold with no timer must not be a hold with no
                // voice: say so once per `JUMBO_GATE_HOLD_LOG_NS`, never per
                // poll, and not before the first interval — a healthy restart
                // proves the rung in a second and must log no hold at all.
                // `jumbo_hold_log_ns == 0` is "armed, interval not started".
                match self.jumbo_hold_log_ns {
                    0 => self.jumbo_hold_log_ns = self.pass_mono_ns + JUMBO_GATE_HOLD_LOG_NS,
                    d if self.pass_mono_ns >= d => {
                        self.jumbo_hold_log_ns = self.pass_mono_ns + JUMBO_GATE_HOLD_LOG_NS;
                        self.log_join_gate_holding(committed, &q);
                    }
                    _ => {}
                }
                false
    ...
    ///   commit acks from (`APPEND_POSITION` and `VOTE` are pairwise-sealed
    ///   exactly as `PROBE` is), so nothing a pass would unlock is reachable
    ///   before then.
    /// - A LEARNER passes on ONE proven voter. It has no vote and needs no
    ///   quorum for anything: its frames come from the leader, a voter, and
    ///   its own report goes back to the leader alone. Demanding a majority
    ///   would hold a standby that can reach only the leader at `/readyz` 503
    ///   for the life of the process, for a guarantee it can never use. What
    ///   the one proof establishes is that this host's own interface carries
    ///   the rung and at least one voter path does; a narrow path to some
    ///   OTHER voter is the refusal check's business if it answers, and the
    ///   same accepted residual a voter has to a non-quorum member if not.
    ///
    /// `voters == 1` with a pending gate is a one-voter cluster WITH learners
    /// (a node with no peers never arms: `own_min_rung` answers `MTU_BOUND`
    /// on an empty map); `0 + 1 > 0` passes it, and the learners' paths are
    /// the refusal check's business, checked first. Off the steady path (a
    /// pending gate, at most every [`JUMBO_GATE_POLL_NS`]), so the `Vec` is
    /// fine.
    #[inline(never)]
    fn jumbo_quorum(&self, committed: u32) -> JumboQuorum {
        let mut voter_addrs: Vec<SocketAddr> = self
            .peers
            .iter()
            .filter_map(|id| self.id_to_addr.get(id).copied())
            .collect();
        voter_addrs.sort_unstable();
        voter_addrs.dedup();
    ...
    /// The `Joining` gate is satisfied: latch it and say so once, with the
    /// terms it was made on.
    fn pass_join_gate(&mut self, committed: u32, q: &JumboQuorum) {
        ...
        crate::obs_event!(
            ...
            proven_voters = q.proven_voters as u64,
            voters = q.voters as u64,
            self_vote = q.self_vote as u64
        );
    }

    /// The `Joining` gate is still holding after another
    /// [`JUMBO_GATE_HOLD_LOG_NS`]: the terms it is short of, and every member
    /// short of the rung (`id:carried`, `0` = silent), so the reason for a
    /// `/readyz` 503 is in the CURRENT log, not only in the one
    /// `jumbo_join_gate_armed` record at the start of the hold.
    #[inline(never)]
    fn log_join_gate_holding(&self, committed: u32, q: &JumboQuorum) {
        let offenders = Self::jumbo_offender_list(&self.jumbo_offenders(committed));
        crate::obs_event!(
            Warn,
            "jumbo_join_gate_holding",
            node = self.id as u64,
            committed = committed as u64,
            proven_voters = q.proven_voters as u64,
            voters = q.voters as u64,
            self_vote = q.self_vote as u64,
            offenders = offenders.as_str()
        );
    }

    /// The `Forcing` gate is satisfied: latch it and say so once.
    ...
    /// ordered by member id — so "the first offender" is the same peer on
    /// every node and on every run. A peer the membership maps to no id is
    /// reported as `u32::MAX` and sorts last, which cannot happen for a
    /// configured member and is not worth a second code path.
    #[inline(never)]
    fn jumbo_offenders(&self, want: u32) -> Vec<(NodeId, u32)> {
        let mut out: Vec<(NodeId, u32)> = self
            .probe_table
            .peers()
            .into_iter()
            .filter_map(|addr| {
                let p = self.probe_table.get(addr)?;
                (p.verified < want).then(|| {
                    (
    ...
    /// `id:carried` for every offender, for the log line — the refusal names
    /// the first, but an operator fixing the fabric needs all of them.
    fn jumbo_offender_list(offenders: &[(NodeId, u32)]) -> String {
        offenders
            .iter()
            .map(|(id, carried)| format!("{id}:{carried}"))
            .collect::<Vec<_>>()
            .join(",")
    }

    /// Spec §6: `force_jumbo_frames` is set, the window has expired, and some
    /// peer has not proven [`JUMBO_MIN_RUNG`]. Two refusals, picked by the
    /// FIRST offender: a peer that answered nothing at all is a LIVENESS fact
    /// (`jumbo_peer_silent`) and nothing is known about its MTU; a peer that
    /// answered below the rung is a path fact (`jumbo_path_too_narrow`).
    #[inline(never)]
    fn jumbo_force_fail_stop(&self, waited: Duration) -> ! {
        let offenders = self.jumbo_offenders(JUMBO_MIN_RUNG);
        let list = Self::jumbo_offender_list(&offenders);
        let waited_secs = waited.as_secs();
        // `own_min_rung` is below the rung, so there is at least one — but a
        // report that panics while explaining a panic is no report at all.
        ...
        if carried == 0 {
            ...
            panic!(
                "consensus fatal (fail-stop): JumboPeerSilent peer={peer} waited_secs={waited_secs} \
                 — force_jumbo_frames is set and member {peer} never answered a probe at any rung, \
                 so nothing is known about its path. This is a LIVENESS failure, not an MTU one: \
                 start the member, or fix what is dropping UC's traffic to it, then restart this \
                 node. Peers short of {JUMBO_MIN_RUNG} (id:carried): {list}"
            );
        }
        crate::obs_event!(
            Error,
            "jumbo_path_too_narrow",
            node = self.id as u64,
            peer = peer as u64,
        ...
        panic!(
            "consensus fatal (fail-stop): JumboPathTooNarrow peer={peer} carried={carried} \
             required={JUMBO_MIN_RUNG} — force_jumbo_frames is set and the path to member {peer} \
             carries only {carried} B datagrams. Raise the interface/path MTU to at least \
             {JUMBO_MIN_RUNG} B end to end (see docs/how-to/jumbo-frames.md), or unset \
             force_jumbo_frames to run at the discovered rung. Peers short of {JUMBO_MIN_RUNG} \
             (id:carried): {list}"
        );
    }
        ...
        for _ in 0..QUERY_DRAIN_PER_CYCLE {
            ...
            match self.query_ring.try_read(&mut buf) {
                Ok(Some(rec)) => {
                    ...
                    //
                    // `jumbo_gate_open` (review minor 5): a pending startup
                    // gate means this node has not proven it can carry the
                    // cluster's datagram budget, so it confirms no read either.
                    let can_serve =
                        self.jumbo_gate_open && self.sm.can_serve() && !self.halt_removed;
                    #[cfg(feature = "mutation-testing")]
    ...
    fn advance_pending_reads(&mut self) -> bool {
        ...
        // `jumbo_gate_open` (review minor 5): and sweep them when a startup
        // gate closes mid-flight — an in-flight read is retried, which is
        // side-effect-free, exactly as on lost leadership.
        let can_serve = self.jumbo_gate_open && self.sm.can_serve() && !self.halt_removed;
        ...
        while i < self.pending_reads.len() {
            ...
            if self.pending_reads[i].phase == ReadPhase::AwaitApplied {
                ...
                let ready = {
                    ...
                    // `uc_service::apply::drain_queries`). A fresh cnc page
    ...
    fn handle_admin(&mut self, req: AdminReq) {
        ...
        if let Some(old) = self.pending_admin_fwd.take() {
            eprintln!("uc_node: admin forward superseded by newer request");
            let version = self.cnc.config_version();
            let (status, reason) = self.audit_admin(
                old.actor.as_deref(),
                AuditOrigin::Local,
    ...
    fn verify_admin(&self, req: &AdminReq) -> Result<Option<String>, u32> {
        ...
        if !uc_crypto::admin::verify(key, &m, &auth.tag) {
    ...
    /// (mirroring the uc_sim driver's work queue). Also drives the intake-gate
    ...
    fn exec(&mut self, act: Action, _work: &mut Vec<Event>) {
        match act {
            ...
            Action::AdvanceCommit { commit } => {
                ...
                // deleted uc_net's two legacy sites (the sender's self-ranking
                // tracker and the receiver's local COMMIT_POSITION store) —
                // grep-provable: `grep -rn "commit.store_release" uc_net/ |
                // wc -l` == 0.
    ...
    /// (`uc_net/src/receiver.rs`, the `DGRAM_KIND_APPEND_POSITION` send); this
...
fn rederive_term_map(
    archive: &Archive,
    recovered: &TermMap,
) -> Result<Vec<(u32, u64)>, uc_log::archive::ArchiveError> {
...
///    Revert it to its own prev level (mirrors `uc_sim::World::on_restart`'s
...
pub(crate) fn rederive_config(
    archive: &Archive,
    rec: ConfigRecord,
) -> Result<ConfigRecord, uc_log::archive::ArchiveError> {
...
fn eventual_fsync_interval_from_env(
    ...
    match std::env::var("UC2_JOURNAL_EVENTUAL_FSYNC_MS") {
        ...
        Ok(v) => {
            if durability != uc_log::Durability::Eventual {
                eprintln!(
                    "uc_node: UC2_JOURNAL_EVENTUAL_FSYNC_MS={v} ignored — \
...
fn journal_durability_from_env() -> Result<uc_log::Durability, String> {
    match std::env::var("UC2_JOURNAL_DURABILITY") {
        Err(std::env::VarError::NotPresent) => Ok(uc_log::Durability::Consistent),
        Err(e) => Err(format!("UC2_JOURNAL_DURABILITY unreadable: {e}")),
        Ok(v) => match v.to_ascii_lowercase().as_str() {
            "" | "consistent" => Ok(uc_log::Durability::Consistent),
            "eventual" => {
                eprintln!(
                    "uc_node: UC2_JOURNAL_DURABILITY=eventual — journal fsync is \
                     ASYNCHRONOUS; acked positions may be lost on power failure \
                     (durability by replication). Opt-in posture; not the default."
                );
                Ok(uc_log::Durability::Eventual)
            }
            other => Err(format!(
...
mod tests {
    ...
    // ---- jumbo (spec §6/§5.4): the two startup gates ----

    /// Is the cnc page's `CAN_SERVE` bit set? `/readyz` reads THIS, not
    /// `Node::can_serve`, so both have to move together.
    fn cnc_can_serve(h: &Harness) -> bool {
        h.cons.cnc.status().flags.load_acquire() & NODE_FLAG_CAN_SERVE != 0
    }

    /// Arm the force gate the way `Consensus::start_with` does for a
    ...
    /// `spend_fast_ladder` for a NARROW path rather than a silent one: every
    /// round goes out AND the peer answers the refresh rung, which is what the
    /// real ladder does on a path that carries `rung` and drops the rungs above
    /// it. `ProbeTable::narrow_peers` requires that current answer — a peer that
    /// answered once and then went quiet is silent, not narrow (the
    /// crypto-restart regression), so a test that only spends the ladder models
    /// a DEAD peer and must not expect a refusal.
    fn spend_fast_ladder_answering(h: &Harness, peers: &[SocketAddr], rung: u32) {
        let cadence = uc_net::probe::ProbeCadence::default();
        for i in 0..=cadence.fast_attempts as u64 {
            h.cons.probe_table.due(i * 2 * cadence.fast_ns);
            for &p in peers {
                h.cons.probe_table.on_ack(p, rung, rung);
            }
        }
    }
    ...
    /// until every peer has proven `JUMBO_MIN_RUNG`, and both the in-process
    /// flag and the cnc bit move together.
    #[test]
    fn the_force_gate_holds_serving_until_every_peer_proves_the_rung() {
        let mut h = harness();
        drive_to_serving_leader(&mut h);
        let peers = mtu_peers(&h);
        h.cons.probe_table.set_peers(&peers);
        ...
        assert!(h.cons.sm.can_serve(), "nor the SM's own view");

        // One peer short is still short (`own_min_rung` is a MIN).
        h.cons
            .probe_table
    ...
    /// Review minor 5: "holds serving" was ADVISORY on a leader. `can_serve`
    /// and the cnc bit were masked off the gate, but step 3's `serving` and the
    /// read barrier were not — so a gate-pending leader went on appending
    /// client commands, firing timers and answering linearizable reads for a
    /// client whose own `serving_gate` said false. One bool in both places.
    #[test]
    fn a_gate_pending_leader_appends_no_client_command_and_answers_no_read() {
        let mut h = harness();
        drive_to_serving_leader(&mut h);
        let peers = mtu_peers(&h);
        h.cons.probe_table.set_peers(&peers);
        ...
        pass_checking_the_gate(&mut h);
        assert!(!h.cons.jumbo_gate_open, "the gate is pending");
        assert_eq!(
            h.cons.cnc.counters().append.load_acquire(),
            before,
            "a held leader must not append a client command"
        );

        // The gate clears: the SAME queued payload is appended now, so the
        // assert above is about the gate and not about a lost payload.
        for &peer in &peers {
            h.cons
                .probe_table
                .on_ack(peer, JUMBO_MIN_RUNG, JUMBO_MIN_RUNG);
        }
        pass_checking_the_gate(&mut h);
        assert_eq!(h.cons.jumbo_gate, Some(JumboGate::Passed));
        assert!(
            h.cons.cnc.counters().append.load_acquire() > before,
            "and appends it once serving"
        );
    }

    /// Spec §6 + `ProbeTable::own_min_rung`: a SOLO node passes at once. It has
    /// no peer whose path could be narrow, and the flag must not make a
    /// one-node cluster unstartable.
    #[test]
    fn a_solo_node_passes_the_force_gate_at_once() {
        let mut h = harness();
        drive_to_serving_leader(&mut h);
        h.cons.probe_table.set_peers(&[]);
        arm_force_gate(&mut h);
    ...
    /// Spec §6: the window is what turns a pending gate into a refusal, and
    /// the deadline is taken from the pass's one monotonic reading.
    #[test]
    fn the_force_gate_arms_its_window_on_the_first_pass_that_evaluates_it() {
        let mut h = harness();
        let peers = mtu_peers(&h);
        h.cons.probe_table.set_peers(&peers);
        arm_force_gate(&mut h);

        h.cons.pass_mono_ns = 7_000;
        assert!(!h.cons.check_jumbo_gate(), "pending");
        assert_eq!(
            h.cons.jumbo_gate,
            Some(JumboGate::Forcing {
                deadline_ns: 7_000 + 30_000_000_000
            }),
            "armed from pass_mono_ns, not from a clock read of its own"
        );
        // A second pass inside the window neither re-arms nor refuses.
        h.cons.pass_mono_ns += 29_000_000_000;
    ...
    /// Spec §6: every peer answered, all of them below the rung → a PATH fact,
    /// naming the lowest-id offender and listing every one of them.
    #[test]
    #[should_panic(expected = "JumboPathTooNarrow peer=0 carried=1408 required=8832")]
    fn the_force_gate_refuses_a_path_that_carries_only_the_baseline() {
        let mut h = harness();
        let peers = mtu_peers(&h);
        all_answer(&h, &peers, MTU_DEFAULT as u32, MTU_DEFAULT as u32);
        arm_force_gate(&mut h);
        ...
        h.cons.pass_mono_ns = 1_000;
        assert!(!h.cons.check_jumbo_gate(), "pending, window armed");
        h.cons.pass_mono_ns += 30_000_000_000;
        h.cons.check_jumbo_gate();
    }

    /// Spec §6: a peer that answered NOTHING is a liveness fact, and it is the
    /// first offender that picks the refusal — peer 0 carries the rung here, so
    /// the silent peer 2 is the one named.
    #[test]
    #[should_panic(expected = "JumboPeerSilent peer=2 waited_secs=30")]
    fn the_force_gate_refuses_a_silent_peer_as_a_liveness_fact() {
        let mut h = harness();
        let peers = mtu_peers(&h);
        h.cons.probe_table.set_peers(&peers);
        h.cons
    ...
    fn an_answered_narrow_path_past_the_fast_ladder_refuses_the_join() {
        ...
        let peers = mtu_peers(&h);
        all_answer(&h, &peers, MTU_DEFAULT as u32, MTU_DEFAULT as u32);
        // Every round goes out and the peers keep answering the refresh rung —
        // a path that carries 1408 and drops the jumbo rungs. The ANSWERING
        // half is load-bearing now: a peer that stopped answering is silent.
        spend_fast_ladder_answering(&h, &peers, MTU_DEFAULT as u32);
    ...
    /// Spec §5.4 as rebuilt around a QUORUM (the maintainer's call,
    /// 2026-09-12, replacing the unproven pass): a peer that has answered
    /// NOTHING never fail-stops this node, and it never lets the node serve
    /// either. The gate passes when the voters that have proven the committed
    /// rung form a quorum WITH this node — the same condition commit itself
    /// needs — and holds otherwise, with no window: if fewer than a quorum of
    /// voters can answer a probe, they cannot ack an append either, so there
    /// is nothing a pass would let this node do.
    ///
    /// Two histories this test pins against, both on the harness's 3-voter
    /// membership:
    ///
    /// - the original 5 s window REFUSED a node whenever a member was merely
    ///   down, slow or still replaying (a crash loop under
    ///   `Restart=on-failure`);
    /// - the unproven pass that replaced it let a previously-proven member
    ///   whose path degraded while it was down join and serve, leaving a
    ///   mislabelled runtime alert where the spec promised a named refusal.
    ///
    /// One dead voter out of three leaves a quorum of two, so the outage that
    /// forced the unproven pass (a restarted survivor un-ready until the dead
    /// host returned) cannot recur: the survivor proves the rung to the OTHER
    ...
    fn silence_holds_the_join_gate_until_a_quorum_of_voters_proves_the_rung() {
        ...
        // …and their ladders are long spent, which is the state that USED to
        // make silence refuse (review fix 1) and then pass unproven.
        spend_fast_ladder(&h);
        publish_committed_rung(&mut h, MTU_BOUND as u32);
        ...
        assert!(
            ...
            "the raise armed the gate and total silence HOLDS it: {:?}",
            h.cons.jumbo_gate
        );
        assert!(
            text.contains(r#""event":"jumbo_join_gate_armed""#),
            "the arm is recorded: {text}"
        );
        assert!(
            !h.cons.can_serve_flag.load(Ordering::Acquire),
            "no quorum has proven the rung, so the node does not serve"
        );
        assert!(!cnc_can_serve(&h));
        assert!(h.cons.jumbo_gate_pending.load(Ordering::Acquire));

        // There is no window: a minute of silence later it is still holding,
        // still not refused (reaching the assert is the no-refusal half).
        h.cons.pass_mono_ns += 60_000_000_000;
        h.cons.jumbo_check_ns = 0;
        assert!(
            !h.cons.check_jumbo_gate(),
            "silence holds for as long as it lasts — no timer passes it"
        );
        ...
        assert!(
            ...
            "the pass records the quorum it was made on: {text}"
        );
        assert!(
            h.cons.can_serve_flag.load(Ordering::Acquire),
            "and the node serves"
    ...
    /// A MID-LADDER peer — one that answered BELOW the rung and whose ladder
    /// is not spent — is evidence in progress, so it holds serving, and under
    /// the quorum rule that hold has NO window: `JUMBO_GATE_WINDOW` belongs to
    /// the force gate alone. What ends the hold is a proven quorum, and on
    /// three voters ONE proven peer is that quorum, whatever the other peer is
    /// still doing.
    #[test]
    fn a_mid_ladder_hold_has_no_window_and_ends_on_a_proven_quorum() {
        let mut h = harness();
        drive_to_serving_leader(&mut h);
        let peers = mtu_peers(&h);
        // Both peers answered the baseline: mid-ladder, the healthy shape of
        // discovery with the jumbo rungs still in flight.
        all_answer(&h, &peers, MTU_DEFAULT as u32, MTU_DEFAULT as u32);
        publish_committed_rung(&mut h, MTU_BOUND as u32);
        pass_checking_the_gate(&mut h);
        ...
        assert!(
            ...
            "expected an armed Joining gate at 8960, got {:?}",
            h.cons.jumbo_gate
        );
        assert!(
            !h.cons.can_serve_flag.load(Ordering::Acquire),
            "a mid-ladder answer below the rung holds serving"
        );
        assert!(!cnc_can_serve(&h));
        assert!(h.cons.jumbo_gate_pending.load(Ordering::Acquire));

        // Well past what the old window was: still holding, still not
        // refused. Reaching the assert is the no-refusal half.
        h.cons.pass_mono_ns += 2 * JUMBO_GATE_WINDOW.as_nanos() as u64;
        h.cons.jumbo_check_ns = 0;
        assert!(
            !h.cons.check_jumbo_gate(),
            "twice the force gate's window later: no timer passes a join gate"
        );

        // One peer proving the rung IS enough: self + 1 of 3 voters.
        h.cons
            .probe_table
            .on_ack(peers[0], MTU_BOUND as u32, MTU_BOUND as u32);
        pass_checking_the_gate(&mut h);
    ...
    /// The crypto-restart state (review round 3): a peer that acked 1408 and
    /// then went stale — a killed member, or one restarted under wire crypto
    /// whose pairwise session no longer opens our sealed probes — keeps
    /// `verified = 1408` forever, on the race ordering where the acks land
    /// BEFORE the node learns the committed rung (`verified: 1408,
    /// advertised: 8960`, the ledger the failing capstone run actually
    /// showed). A stale-acked peer
    /// is SILENT: it neither refuses nor counts toward the quorum, in either
    /// direction — it is not proven-narrow, and a 1408 ack is not proof of
    /// 8960 — so the gate simply waits for a quorum that IS proven.
    #[test]
    fn a_stale_acked_peer_neither_refuses_nor_counts_toward_the_quorum() {
        let mut h = harness();
        drive_to_serving_leader(&mut h);
        let peers = mtu_peers(&h);
        ...
        assert!(
            ...
            "an outlived ack is silence: no refusal, and no quorum either: {:?}",
            h.cons.jumbo_gate
        );
        assert!(!h.cons.can_serve_flag.load(Ordering::Acquire));
        assert!(h.cons.jumbo_gate_pending.load(Ordering::Acquire));

        // One of them comes back (a fresh session) and proves the rung: a
        // quorum with self, and the gate passes.
        h.cons
            .probe_table
            .on_ack(peers[0], MTU_BOUND as u32, MTU_BOUND as u32);
        pass_checking_the_gate(&mut h);
    ...
    /// The quorum rule decides pass versus hold and NOTHING about refusal: a
    /// peer that is proven narrow — it is ANSWERING below the rung and its
    /// fast ladder is spent — still fail-stops the node IMMEDIATELY, however
    /// long the hold has lasted, and (the second test) even when the OTHER
    /// voters already form a proven quorum with this node. Spec §5.4's
    /// promise is about every path the log's frames must cross, and a member
    /// that cannot receive them is a named refusal, not a lagging-peer alert.
    #[test]
    #[should_panic(expected = "PathBelowCommittedMtu peer=0 committed=8960 carried=1408")]
    fn a_proven_narrow_peer_refuses_however_long_the_hold_has_lasted() {
        let mut h = harness();
        drive_to_serving_leader(&mut h);
        let peers = mtu_peers(&h);
        // Mid-ladder to begin with, so the gate holds; one round has gone out
        // and the baseline ack came back.
        all_answer(&h, &peers, MTU_DEFAULT as u32, MTU_DEFAULT as u32);
        publish_committed_rung(&mut h, MTU_BOUND as u32);
        pass_checking_the_gate(&mut h);
        assert!(
            ...
            "expected an armed Joining gate, got {:?}",
            h.cons.jumbo_gate
        );

        // A long time goes by mid-ladder — no refusal, no pass.
        h.cons.pass_mono_ns += 2 * JUMBO_GATE_WINDOW.as_nanos() as u64;
        h.cons.jumbo_check_ns = 0;
        assert!(
            !h.cons.check_jumbo_gate(),
            "mid-ladder holds, never refuses"
        );

        // Then the ladder runs out while the peers keep answering the refresh
        // rung and nothing above it: proven narrow, refused on the spot.
        spend_fast_ladder_answering(&h, &peers, MTU_DEFAULT as u32);
        h.cons.jumbo_check_ns = 0;
        h.cons.check_jumbo_gate();
    }

    /// The refusal is checked BEFORE the quorum, on every poll: a proven
    /// quorum does not excuse a member this node provably cannot carry the
    /// committed rung to. Peer 0 has proven 8960 (a quorum with self on three
    /// voters); peer 2 answers 1408 and nothing above it across its whole fast
    /// ladder.
    ///
    /// Not watched red: the shipped gate already refused here (its all-peers
    /// minimum was below the rung, so the narrow check ran first). This pins
    /// the ordering the rewrite must keep.
    #[test]
    #[should_panic(expected = "PathBelowCommittedMtu peer=2 committed=8960 carried=1408")]
    fn a_proven_narrow_peer_refuses_even_when_the_other_voters_are_a_proven_quorum() {
        let mut h = harness();
        drive_to_serving_leader(&mut h);
        let peers = mtu_peers(&h);
        h.cons.probe_table.set_peers(&peers);
        // Every round goes out to both; peer 2 (peers[1]) answers only the
        // baseline, peer 0 answers the top rung once and is proven.
        spend_fast_ladder_answering(&h, &peers[1..], MTU_DEFAULT as u32);
        h.cons
            .probe_table
            .on_ack(peers[0], MTU_BOUND as u32, MTU_BOUND as u32);
        publish_committed_rung(&mut h, MTU_BOUND as u32);
    ...
    /// A two-voter cluster's quorum is both voters, so the OTHER voter must
    /// prove the rung before this node serves — and with that voter down the
    /// cluster has no quorum for anything, so the hold costs nothing it did
    /// not already lose. The harness's genesis voters are `{0, 1, 2}` with
    /// this node `1`; voter 2 is removed the way a committed membership frame
    /// removes it.
    #[test]
    fn a_two_voter_cluster_needs_the_other_voter_proven() {
        let mut h = harness();
        drive_to_serving_leader(&mut h);
        adopt_config_change(&mut h, ConfigOp::RemoveVoter { id: 2 });
        assert_eq!(h.cons.peers, vec![0], "voters are now {{0, 1}}");
        let other = h.cons.id_to_addr[&0];
        h.cons.probe_table.set_peers(&[other]);
        spend_fast_ladder(&h);
        publish_committed_rung(&mut h, MTU_BOUND as u32);
        pass_checking_the_gate(&mut h);
        ...
        assert!(
            ...
            "self alone is 1 of 2, not a quorum: {:?}",
            h.cons.jumbo_gate
        );
        assert!(!h.cons.can_serve_flag.load(Ordering::Acquire));
        ...
        h.cons
            .probe_table
            .on_ack(other, MTU_BOUND as u32, MTU_BOUND as u32);
        pass_checking_the_gate(&mut h);
        assert_eq!(h.cons.jumbo_gate, Some(JumboGate::Passed));
        assert!(h.cons.can_serve_flag.load(Ordering::Acquire));
    }

    /// Four voters need three, so self plus ONE proven peer is not a quorum
    /// and self plus TWO is. Voter 3 joins as a learner and is promoted, the
    /// only route a voter can take into a running cluster.
    #[test]
    fn a_four_voter_cluster_needs_two_proven_peers() {
        let mut h = harness();
        drive_to_serving_leader(&mut h);
        let addr3: SocketAddr = "127.0.0.1:9103".parse().unwrap();
        adopt_config_change(
            &mut h,
            ConfigOp::AddLearner {
                id: 3,
                addr: addr_to_pair(addr3),
            },
        );
        adopt_config_change(&mut h, ConfigOp::PromoteLearner { id: 3 });
        let mut voters = h.cons.peers.clone();
        voters.sort_unstable();
        assert_eq!(voters, vec![0, 2, 3], "voters are now {{0, 1, 2, 3}}");
        let peers: Vec<SocketAddr> = voters.iter().map(|id| h.cons.id_to_addr[id]).collect();
        h.cons.probe_table.set_peers(&peers);
        spend_fast_ladder(&h);
        publish_committed_rung(&mut h, MTU_BOUND as u32);
        pass_checking_the_gate(&mut h);
    ...
    /// A joining LEARNER has no vote and needs no quorum: its frames come
    /// from the leader, a voter, so ONE proven voter passes it (a majority
    /// would hold a standby that can reach only the leader at `/readyz` 503
    /// forever — the review's finding). This node is demoted the way a
    /// committed membership frame demotes it. A learner never leads, so
    /// `can_serve` is not the observable here; the gate state and the
    ...
    fn a_joining_learner_needs_one_proven_voter_not_a_majority() {
        ...
        assert!(
            h.cons.sm.config().is_learner(1),
            "this node is a learner now"
        );
        let peers = mtu_peers(&h);
        h.cons.probe_table.set_peers(&peers);
        spend_fast_ladder(&h);
        ...
        assert!(
            ...
            "both voters silent: a learner has proven nothing to anyone: {:?}",
            h.cons.jumbo_gate
        );
        assert!(h.cons.jumbo_gate_pending.load(Ordering::Acquire));
        ...
        assert!(
            text.contains(r#""event":"jumbo_gate_passed""#)
                && text.contains(r#""gate":"join""#)
                && text.contains(r#""proven_voters":1,"voters":2"#)
                && text.contains(r#""self_vote":0"#),
            "the pass records the learner's terms: {text}"
        );
        assert!(!h.cons.jumbo_gate_pending.load(Ordering::Acquire));
    }

    /// The refusal is about EVERY member's path, not only voters': a LEARNER
    /// peer that is answering below the committed rung with its ladder spent
    /// fail-stops this node even though the voters form a proven quorum. A
    /// voter-only filter on the narrow check would be the natural mistake one
    /// statement after the voter-only quorum count.
    #[test]
    #[should_panic(expected = "PathBelowCommittedMtu peer=7 committed=8960 carried=1408")]
    fn a_narrow_learner_peer_refuses_even_under_a_proven_quorum() {
        let mut h = harness();
        drive_to_serving_leader(&mut h);
        ...
        adopt_config_change(
            ...
            ConfigOp::AddLearner {
                id: 7,
                addr: addr_to_pair(addr7),
            },
        );
        let voters = mtu_peers(&h);
        let mut all = voters.clone();
        all.push(addr7);
        h.cons.probe_table.set_peers(&all);
        // Every round goes out to all three; the learner keeps answering the
        // baseline and nothing above it, the voters prove the top rung.
        spend_fast_ladder_answering(&h, &[addr7], MTU_DEFAULT as u32);
        for &v in &voters {
            h.cons
                .probe_table
                .on_ack(v, MTU_BOUND as u32, MTU_BOUND as u32);
        }
        publish_committed_rung(&mut h, MTU_BOUND as u32);
        pass_checking_the_gate(&mut h);
    }

    /// A hold with no timer must not be a hold with no voice: while the
    /// `Joining` gate holds, the node repeats a `Warn` record every
    /// [`JUMBO_GATE_HOLD_LOG_NS`] naming the committed rung, the quorum it is
    /// short of and every member short of the rung, so an operator reading a
    /// log that has long since rotated past `jumbo_join_gate_armed` still sees
    ...
    /// voters. Learner 7 proves the rung while both voters stay silent, and
    ...
    fn a_learner_peers_proof_does_not_count_toward_the_quorum() {
        ...
        h.cons
            .probe_table
            .on_ack(addr7, MTU_BOUND as u32, MTU_BOUND as u32);
        publish_committed_rung(&mut h, MTU_BOUND as u32);
        pass_checking_the_gate(&mut h);
        assert!(
            matches!(
            ...
            "a learner's proof is not a voter's: {:?}",
            h.cons.jumbo_gate
        );
        assert!(!h.cons.can_serve_flag.load(Ordering::Acquire));
        ...
        assert!(
            text.contains(r#""proven_voters":1,"voters":3"#),
            "the learner is in neither the count nor the denominator: {text}"
        );
        assert!(h.cons.can_serve_flag.load(Ordering::Acquire));
    }

    /// Adopt a membership change the way a committed `CLUSTER kind=Membership`
    /// frame is adopted: the kernel through `ConfigObserved`. `feed` executes
    /// the resulting `ConfigAdopted` inline, which is what rebuilds the
    /// node's peer maps (`self.peers`, `id_to_addr`) and re-seeds the probe
    /// table; the trailing pass only lets the cnc flags catch up.
    fn adopt_config_change(h: &mut Harness, op: ConfigOp) {
        let next = h
            .cons
            .sm
            .config()
    ...
    fn a_higher_committed_rung_re_arms_the_gate_on_a_serving_node() {
        ...
        // The peers are re-answered at 8832 (mid-ladder against the new rung);
        // silence would hold it just the same, which is
        // `silence_holds_the_join_gate_until_a_quorum_of_voters_proves_the_rung`.
        all_answer(&h, &peers, 8832, 8832);
        publish_committed_rung(&mut h, MTU_BOUND as u32);
        pass_checking_the_gate(&mut h);
        assert!(
    ...
    /// arms NOTHING. The leader raises the rung only once every member has
    /// advertised it, so an adopting node's own minimum is already at or above
    /// what it adopts — and a gate that fired here would fail-stop a healthy
    /// cluster the first time it discovered a jumbo path.
    #[test]
    fn a_raise_this_node_has_already_proven_arms_no_gate() {
        let mut h = harness();
        drive_to_serving_leader(&mut h);
        let peers = mtu_peers(&h);
        all_answer(&h, &peers, MTU_BOUND as u32, MTU_BOUND as u32);
    ...
    /// Spec §5.4, the other half: a MEMBERSHIP CHANGE after the raise drops
    /// `own_min_rung` to 0 (a fresh peer is unresolved until it answers) while
    /// moving no rung — so it arms nothing either. Without the raise edge this
    /// is the case that would fail-stop every healthy voter five seconds after
    /// an `add-learner` on a jumbo cluster.
    #[test]
    fn a_membership_change_after_the_raise_arms_no_gate() {
        let mut h = harness();
        drive_to_serving_leader(&mut h);
        let peers = mtu_peers(&h);
        all_answer(&h, &peers, MTU_BOUND as u32, MTU_BOUND as u32);
        ...
        let mut with_learner = peers.clone();
        with_learner.push("127.0.0.1:9199".parse().unwrap());
    ...
    /// Spec §6: `PathBelowCommittedMtu` "still applies under the flag and takes
    /// precedence". The join gate REPLACES a pending force gate (its window is
    /// the shorter one, and its rung is the stricter fact), and passing it
    /// satisfies both — every rung above the baseline is at or above
    /// `JUMBO_MIN_RUNG`.
    #[test]
    fn the_join_gate_takes_precedence_over_the_force_gate() {
        let mut h = harness();
        drive_to_serving_leader(&mut h);
        let peers = mtu_peers(&h);
        all_answer(&h, &peers, MTU_DEFAULT as u32, MTU_DEFAULT as u32);
        ...
        publish_committed_rung(&mut h, MTU_BOUND as u32);
        // The peers have answered the baseline and nothing above it, and their
        // ladders are not spent: mid-ladder, so the join gate arms and HOLDS
        // rather than refusing — `all_answer` below is what clears it.
        pass_checking_the_gate(&mut h);
        assert!(
            matches!(
                h.cons.jumbo_gate,
    ...
    /// those reports into `commit` (`uc_net/src/receiver.rs`, the
    ...
    // ==================================================================
    //
    // Fixtures mirror `uc_net::receiver`'s crypto fixtures (same key-file
    ...
    use uc_crypto::schedule::epoch_is_newer;
    ...
    /// Standard-alphabet base64 with padding — matches `uc_crypto::identity`'s
    ...
    struct CryptoHarness {
        ...
        peer_recv: uc_crypto::ReceiveHalf,
        peer_sock: UdpSocket,
