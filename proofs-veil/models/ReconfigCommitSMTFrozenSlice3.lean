...
veil module UcReconfigCommitSMTFrozenSlice3
...
-- ---------- SLICE S3 (session 6): certify T18 / T13 / T14 at ALL actions + INIT ----------
-- Model UNCHANGED (35 requires / 11 assumptions, ghost `commitElecQuorum` included).
-- Run 28 certified these three at `becomeLeader` in the FULL bundle; this slice covers the
-- other nine actions and the initialisation obligation. HYPOTHESIS SET (transfer audit):
-- role_exclusive, cand_cfg_frozen, eleccfg_not_ahead, reach_bound, reach_mono,
-- cfg_from_genesis, self_vote, grant_state, leader_reach_strict, elecq_witness,
-- elecq_grant_covers_reach, commit_leader_unique, commit_leader_only_after_commit,
-- commit_leader_at_commit_cfg, commitq_witness, commitq_grant_covers_reach,
-- commit_leader_frozen_reach -- all members of the full bundle.
...
-- ADDED IN S3b: the two clauses T13's preservation at `propose`/`adopt` consumes for the
-- V = commit-leader corner -- together they give `committedTerm <= curTerm (commit leader)`,
-- without which the mover's-term bound on a re-stamped `reachAt` cannot be closed.
...
-- ADDED IN S3c: run 33's remaining T13 CTI at propose/adopt had `committed = true` with
-- `isCommitLeader = []` -- a state no reachable run can produce (`commitEntry` writes both in
-- one step) and which makes every commit-leader clause VACUOUS, so nothing bounds
-- `commitElecQuorum` members' terms. `commit_leader_evidence` is the full-bundle clause that
-- supplies the commit leader; its absence was the artifact.
