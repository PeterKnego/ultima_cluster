...
-- T41 (probe 2's addition; truth argument in ledger item 67): the commit leader's proposal of a
-- config strictly above the commit config necessarily POSTDATES the commit — `cfgOf` is
-- forward-only and `commitEntry` stamps `commitCfgid := cfgOf i`, so a proposal made before the
-- commit would have moved `cfgOf` above `commitCfgid` and the commit would then have stamped
-- the HIGHER config. Postdating the commit, the proposer held E (`commitEntry` requires
-- `holdsE i`, and `holdsE` is monotone), so `propAfterE` was set.
invariant [commit_leader_prop_after]
  (isCommitLeader I ∧ hasProposal I ∧ cfgLt commitCfgid (proposedC I)) → propAfterE I

invariant [committed_above_backed]
  (committed ∧ cfgCommitted D ∧ cfgLt commitCfgid D) → cfgBacked D

#gen_spec

#check_action commitCfg
