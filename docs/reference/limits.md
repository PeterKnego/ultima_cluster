...
| The schedule table is **not carried in the snapshot stream**. With purge ON, a below-floor joiner whose table frame was purged runs without the table until the next `uc2ctl schedule apply`. Its `uc2_schedule_table_position` reads `0` while every other node reads the adopted position — which is exactly what `Uc2ScheduleTableDiverged` catches. It is not only that node's problem: only the leader appends timer frames, so **if that node later wins an election, every scheduled recurrence in the cluster stops** until an operator re-applies | [Log time and timers, explained § The schedule table](../notes/uc2-log-time-and-timers-explained.md#known-limits-of-the-table), [`docs/BACKLOG.md` § 2a](../BACKLOG.md) |
