...
#   1. Applies tc-netem on node0's uplink via SSH (idempotent del-first).
...
#   5. Removes netem from node0 (also in a cleanup trap so it's torn down on
#      any exit path).
#
...
# netem helpers
# ---------------------------------------------------------------------------
# netem_apply <node0_ip> <delay_ms> <loss_pct>
# Idempotent: del-first (|| true), then add only when shaping is needed.
netem_apply() {
...
# ---------------------------------------------------------------------------
# Cleanup trap: always remove netem from node0 on EXIT/INT/TERM.
# (The per-experiment finally-block also removes it, but this is the belt.)
# ---------------------------------------------------------------------------
cleanup() {
  if [[ "${_NETEM_ACTIVE:-0}" -eq 1 ]]; then
    echo "[cleanup] removing netem from node0 (${NODE0_IP})" >&2
    netem_remove "${NODE0_IP}"
    _NETEM_ACTIVE=0
  fi
}
  ...
  for p in $PAYLOADS; do
    for d in $NETEM_DELAYS; do
      for l in $NETEM_LOSS; do
        ...
        # --- 1. Apply netem on node0 ---
        _NETEM_ACTIVE=1
        netem_apply "${NODE0_IP}" "$d" "$l"
...
[[ "$DRY_RUN" -eq 0 ]] && echo "    results: ${RESULTS_TSV}" >&2
