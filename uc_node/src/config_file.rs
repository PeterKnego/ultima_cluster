...
/// `[admin].keys`. Not the loaded [`uc_crypto::admin::AdminKey`] — this is
...
/// [`uc_crypto::admin::AdminPolicy`], which this maps onto in the daemon.
...
/// the `uc_node_toml` fuzz target) can reach the parser without staging a
    ...
    match admin.auth {
        ...
        AdminAuthMode::Hmac => {
            ...
            for k in &admin.keys {
                ...
                let h = uc_crypto::admin::fnv1a64(&k.name);
...
mod tests {
    ...
    /// `uc_node_toml` fuzz target drives; if it ever stopped being the same
