...
| `uc2ctl backup` does not copy `snapshots/cluster/`, so a restore rebuilds the cluster FSM from its genesis seed plus the restored journal's `CLUSTER` frames — correct wherever the journal still reaches genesis, and a re-`apply` of the table and settings otherwise | [Instance directory § Durability classes](instance-directory.md#durability-classes) |
