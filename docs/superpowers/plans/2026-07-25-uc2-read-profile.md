...
- Sweep the ladder by invoking `read_profile client` **on the leader's host** (shmem is same-host only), once per rung: `--readers` from the sweep list × `--mode {lin,snap}` × **`--write-rate {0, W}` — both mixes are required, because the reformulated clause (b) is evaluated on the READ-ONLY arm** (see below). Optionally pass `--node-pid` and `--service-pid` (both roles print their pid at startup) — these control only the *diagnostic* agent yield-rate sample and do **not** affect the decision rule.
- **Do NOT re-implement the decision rule in Python.** Each `client` run prints one machine-readable rung JSON line to stdout; collect those lines into a file and run `read_profile decide --rungs FILE` to evaluate them through the *same* `evaluate_decision_rule` the unit tests pin. A rule re-implemented outside its tests is no longer a pre-commitment.
- **Treat any rung with `inflight_at_end != 0` as INVALID** (spec §6, threat 6), not merely suspicious. The `client` role already refuses to emit a rung JSON line for an invalid run and exits nonzero — so the orchestrator must **check exit codes** and not simply tee stdout, or it will silently drop rungs it should be reporting as failures.

**Note on clause (b), amended 2026-07-25 before any data existed (spec §2.1):** the original occupancy-ranking clause (b) was abandoned — the `/proc` yield-rate proxy was measured non-functional (`sched_yield` leaves the task `TASK_RUNNING`, so it never increments `voluntary_ctxt_switches`; 2,000,000 yields → +0). Clause (b) now requires the gap to be present in the **read-only arm**, with the client sustaining ≥90% of target concurrency and neither arm degraded. Agent occupancy is diagnostic output only.
