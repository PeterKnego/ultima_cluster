...
veil module UcReconfigCommitSMTT17Slice
...
-- ---------- SLICE S2 (session 6): certify T17 `role_below_meets_quorum` ----------
-- Model UNCHANGED (35 requires / 11 assumptions). HYPOTHESIS SET (transfer audit):
-- role_exclusive, cand_cfg_frozen, eleccfg_not_ahead, reach_bound, reach_mono,
-- cfg_from_genesis, self_vote, grant_state, leader_reach_strict, reach_quorum_below,
-- role_below_quorum_strict (T15), role_below_meets_quorum (T17).
...
-- T17 — THE TARGET. Pre-composes T15 with `adjacent_cfg_quorum_intersection`, so that
-- election_safety's becomeLeader VC never has to find that instantiation itself.
