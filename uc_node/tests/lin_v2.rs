...
//!
//! `RegisterSm` persists NOTHING (see `uc_lincheck`): after a service crash it
...
fn linearizable_under_reconfig_churn() {
    ...
    // A hard ceiling on top of `TARGET_OPS`: the WGL checker (`uc_lincheck`,
...
/// bound (that lives in `uc_service`'s `lag` unit tests and the apply-hop
