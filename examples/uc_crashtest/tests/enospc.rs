...
//! (`uc_journal/src/journal/writer.rs:405-445`) -> the archive agent's
//! `.expect("archive fail-stop")` panics (`uc_node/src/node.rs:1023`) -> the
//! agent's finished-flag flips -> the `uc2-node` daemon's outer loop notices
//! it, logs `agent_failstopped`, and exits `ExitCode::FAILURE`
//! (`uc_node/src/bin/uc2-node.rs:134-145`) -> systemd's
...
//!
//! # Why this spawns the REAL `uc2-node` daemon, not `uc_crashtest-node`
//!
//! This crate's own reference bin (`uc_crashtest-node.rs`) has no
...
//! binary (`uc_node/src/bin/uc2-node.rs`), driven by a TOML config file
//! (`--config`), not `uc_crashtest-node`'s CLI flags. Node 0 in both tests
//! below is therefore the real `uc2-node` daemon; nodes 1/2 (the
//! survivors, never under test) stay on the lighter-weight
//! `uc_crashtest-node`/`uc_crashtest-service` reference bins, exactly as
...
//! `uc2-node` belongs to the `uc_node` package, a dependency of this crate,
//! not this crate itself. [`uc_node_daemon_bin`] below builds it on demand
//! via `cargo build -p uc_node --bin uc2-node --message-format=json` and
...
/// build-and-parse.
fn uc_node_daemon_bin() -> &'static Path {
    static BIN: OnceLock<PathBuf> = OnceLock::new();
    BIN.get_or_init(|| {
        let out = Command::new(env!("CARGO"))
            .args(["build", "-p", "uc_node", "--bin", "uc2-node", "--message-format=json"])
            .output()
            .expect("spawn `cargo build -p uc_node --bin uc2-node`");
        assert!(
            out.status.success(),
            "cargo build -p uc_node --bin uc2-node failed:\n{}",
...
/// `uc_node/src/config_file.rs`'s `NodeConfigFile`. `buffer_bytes` (must be
...
fn spawn_daemon(config_path: &Path) -> Child {
    Command::new(uc_node_daemon_bin())
        .arg("--config")
        .arg(config_path)
        .stdout(Stdio::inherit())
        .stderr(Stdio::piped())
...
fn same_client_retry(e: &uc_client::ClientError) -> bool {
    is_transient(e)
}
...
/// `uc_crashtest-node`/`uc_crashtest-service` reference bins under `tmp`.
/// See [`boot_cluster`].
struct Cluster {
    dirs: Vec<PathBuf>,
    node0_child: Child,
...
/// `JournalError::Clone` (`uc_journal/src/error.rs:33`) rewraps `Io(e)`
