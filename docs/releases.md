...
| the local proof stack: `cargo fmt --all -- --check`, five `clippy` invocations **including the MSRV gate** (`cargo +1.89.0 clippy --workspace --all-targets --locked`), the two fixture builds, `cargo test --workspace`, `cargo test -p uc_diffreplay --test pin_verify`, `lin_v2`, the hard-crash suite, `cargo +nightly fuzz build`, `scripts/check_doc_links.py` | this plan's final task, on the release-prep head | pending — a dev box, and not the whole `docs/VERIFICATION.md` surface |
| `publish-check`'s batched `cargo package --no-verify` over the **fourteen** publishable crates, and `scripts/check_publish_metadata.sh` | run locally with `uc_diffreplay` added to the batch, before the bump | ok — fourteen `Packaged` lines, metadata within crates.io limits |
...
| workspace correctness stack (`lin_v2`, `lin_partition_v2`, `learner`, `elle_check.sh`, hard-crash) | full local run 2026-09-07 on `81192db` (the harness merge; `lin_v2` on `bfa97c0`, the same library code): `cargo test --workspace` 1 708 passed / 0 failed; `lin_partition_v2` 8/8; `lin_v2` (release) 15/15; hard-crash 11/11; `elle_check.sh` clean on all five scenarios under both models; logs `~/scratch/proof-2026-09-07/` | **green locally** — the dev box is not a bench, but correctness tiers are correctness tiers; not a fleet result |
...
| time-and-timers gate, **row d** — the apply-hop A/B for log time and timers (`17d5c6b` → `HEAD`, N=1 and N=2, bounded) | the same gate doc + [`scripts/apply_ab.sh`](/scripts/apply_ab.sh) | **RE-RUN 2026-09-08 with a guardable baseline: N=1 is WITHIN its bar.** The 2026-09-07 run was unadjudicable because `17d5c6b`'s harness predates the `pace_stalls` counter, so Ruling Q9's guard could not be evaluated; a one-field backport of that commit's own harness (`--harness-a`) fixed it, and the guard now evaluates on every arm. **N=1 +0.334 % against a 1.634 % resolution → `within resolution`; N=2 +2.427 % (head FASTER) against a 0.144 % resolution → `outside resolution`.** Neither is a regression, so the flag day's apply hop is free at N=1 and faster at N=2 across the whole span. Caveats: N=1's resolution was unusually wide (1.634 % vs 0.150 % on row f the same week), and N=2's verdict is marginal — it clears Ruling Q8's `signal_bar` by 11 % and would have read `inconclusive (noisy run)` under the earlier rule. Row d is a rebuild-resolution check across a harness asymmetry, never the arm-cost verdict — that is row f's |
