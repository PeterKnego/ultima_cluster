...
**CORRECTION (5-repeat follow-up, 2026-07-01):** the single-run signals above were NOISE. Ran 5 repeats/arm at the 30k rung (inflight=256, independent fresh-cluster runs): **SyncCore mean 24.3k (22.6-26.2k), p99 2294ms, 0/5 collapsed; RaftCore mean 25.5k (24.5-26.2k), p99 1829ms, 0/5 collapsed.** So (a) RaftCore does NOT congestion-collapse at 30k — the earlier one-off 30k->2.6k was a transient (likely an election hiccup), NOT a systematic weakness; SyncCore has **no collapse-resistance advantage**. (b) The single-run p99 edge reversed — RaftCore p99 is marginally *lower* here. **At the operating ceiling (inflight=256/30k) SyncCore and RaftCore are statistically EQUAL, RaftCore marginally ahead on both throughput and p99.** SyncCore's one *measured* real edge remains low-concurrency latency (this doc's inflight-1 +35-45% regime), NOT the ceiling. SyncCore was nonetheless made the DEFAULT model (commit addb8ee, merged to main) on the cleaner-synchronous-foundation + maintainer-direction basis — explicitly NOT on a proven throughput/robustness advantage at load.
...
### 6d. Ceiling correction (2026-07-02): crc32 was NOT the throughput ceiling

Follow-up diagnosis found a second `purge_before` full-segment scan (the `first_seq` recompute; fixed in `28dcd6c`) = 93% of journal decodes = the residual crc32. Eliminating it made the box ~85% idle but left throughput at ~25k (sweep 128/256/512 x2 = 20-30k). **So the ~25k ceiling is the LATENCY FLOOR (fsync + 3-proc IPC + async replication choreography), not CPU/crc32** — which is exactly why SyncCore (a latency play) is throughput-null here: it trims per-op latency but not the fsync/IPC floor. Full write-up: `docs/benchmarks/throughput-ceiling-root-cause-2026-07-01.md` §Correction. To move past ~25k: attack the latency floor (co-location / fewer IPC hops / fsync batching), not consensus or journal CPU.
