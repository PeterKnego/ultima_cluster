...
//! `Vec`s the config itself owns. Dep-free — `uc_consensus` has no path
//! dependencies, so `Addr` is a bare `(u32, u16)` tuple; converting it to a
//! real `SocketAddr` is `uc_node`'s job.

use crate::election::NodeId;

/// (ipv4, port) — deliberately not `SocketAddr`: `uc_consensus` stays
/// dep-free, so the socket-address conversion lives in `uc_node`.
