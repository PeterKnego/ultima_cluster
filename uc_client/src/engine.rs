...
//! the target service's apply boundary — and it belongs to `uc_service`, not
...
/// `uc_node::InstanceDir` — see `uc_node/src/ipc.rs`). Moved here from
...
mod tests {
    ...
    fn make_instance_two_fsms(dir: &Path, app_id: &str) {
        const MIB: u64 = 1 << 20;
        let page = uc_log::cnc::CncPage::create_file(&dir.join(CNC_FILE), &meta(app_id)).unwrap();
