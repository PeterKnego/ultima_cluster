...
**M14b's −4.2 % client-hop cost was not real.** T2 rebuilt both commits fresh: −0.30 % / +0.31 % / −0.05 % with overlapping ranges, and two independent builds of the SAME commit differed by +1.02 %. The M14b-era number was a property of one binary pair (layout), not of the source; the box resolves ~1 %. No variant was measured (the brief's stop condition), `engine.rs` is unchanged, the fast path (T1) is kept for its measured tail win (p90 3 → 2 µs). Recorded in `docs/benchmarks/uc2-m14c-client-hop-2026-08-28.md`, the M14b plan's addendum ("Superseded"), spec §14.2's erratum, and CLAUDE.md's third perf bullet (rebuild the same source twice first).
...
- T2's premise (above). T4's sender code let a BEGIN resend refresh the session's activity clock (a dead peer would have pinned the slot); T4's brief said "20 ms cannot elapse" in a test where the fake socket busy-spins 500 ms. T5's single `current` artifact could not work against a sender that rotates on send, not ack (→ `parts: Vec<SnapPart>`), and the as-built sender needed three receiver mechanisms the brief lacked (placement guard, missing-BEGIN probe, DONE latch). T6's `ids()` would have made every harness node offer an empty set (→ `ring_ids()`/`ring_mask()`). T7's family count (70) missed its own addendum's two counters (72; 73 after the fix wave). T8's "exactly 8192" was one frame low (the door checks before admitting → 8320) and the scenario needed a paced load + a scrape-while-shut gate. T9's brief fixed `CARGO_TARGET_TMPDIR` for a lib test (unavailable). T11's `m10_gate` command lacked its `coverage` subcommand. The brief-level "workspace compiles at every commit" gap (Ruling J) repeats M14b's Ruling B — a plan-writing rule worth keeping: when a task changes a public signature, it owns the call sites.

### Deferred to M14c′ (triaged by the final review)
