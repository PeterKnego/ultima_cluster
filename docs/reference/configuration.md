...
| `UC2_APPLY_IDLE` | `uc_service` | The apply agent's idle strategy: `backoff` (the default: spin 2 000, yield 1 024, then sleep 50 µs — awake across a client round trip after each frame, asleep within a millisecond when truly idle), `spin` (never park — one core per service, lowest response latency), `yield`, or `sleep:<micros>` (the pre-2026-09-16 flat sleep at a chosen cadence). Anything else is refused by name at service start. Per-process and deploy-varying: pegging a core per service is a capacity decision, not cluster state. Added 2026-09-16 for the service-time measurement (`docs/benchmarks/uc2-service-time-2026-09-16.md`), which found the flat 50 µs sleep landing on half of all low-load responses. |
