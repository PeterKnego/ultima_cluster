...
//! `uc2_gateway`: the **edge** — the process that lets a client reach an
...
//! ([`uc2_remote`]) on the node's host and pumps it through one
//! [`uc2_client::Engine`] over that shmem, so a remote client gets the
...
//!
//! ```text
//!   remote client ──TCP (uc2_remote v1)──▶ Edge ──shmem──▶ uc2_node
//! ```
//!
