...
|---|---|---|---|
| a | ONE new client connection through the real edge into the real 3-node cluster (`hop_bench remote-load` rebuilt on the new client, conns=1, inflight 1024) vs the direct `Engine` arm on the same generation | **≥ 0.5× direct** resp/s | **PASS** (fleet 4x c6id.2xlarge, 2026-08-25): 1,079,930/s vs direct 1,751,213/s = **0.617x** (bar >= 0.5x) |
| b | N-connection aggregate through the edge (new client), best rung ≤ 16 | **≥ 0.75× direct** | **PASS** (fleet, 2026-08-25): best rung N=16 1,464,381/s vs direct 1,751,213/s = **0.836x** (bar >= 0.75x); closes M12 row 2 |
| c | Ladder N = 1,2,4,8,16 at 1024 inflight on the co-located host, with the ring fix + edge budget | **monotone, no collapse**: 0 lost, p99 bounded (< 1 s at every rung), no rung > 20% below the previous rung | **PASS** (fleet, 2026-08-25): N=[1,2,4,8,16] = 1.08/1.36/1.33/1.28/1.46 M/s, **lost 0**, p99 max **17.0 ms**, no rung >20% below prev — no collapse |
| d | N local `Engine`s into one node on an oversubscribed host (`engine-load --engines 1,2,4,8` on the 8-vCPU server host, and locally on 4 vCPU) | **≤ linear degradation**: resp/s at N engines ≥ (cores / busy threads) × single-engine resp/s × 0.5, never below 10% of single-engine | **PASS** (fleet 8 cores, 2026-08-25): single-engine 2,442,921/s; N=1:2.44M N=2:3.98M N=4:3.57M N=8:3.86M/s (scales up, <= linear) |
...
## Fleet run — 2026-08-25 (4x c6id.2xlarge, us-east-1)

`python3 bench-infra/scripts/m13_hop_bench.py --fleet --arms gate --secs 10`
adjudicated **RESULT: PASS — 5 rows, 0 points without a RESULT line**. Direct
`Engine` baseline 1,751,213/s. The M12 congestive collapse (~30x drop past the
admission window at N=8) does not appear: the N ladder holds 1.08-1.46 M/s with
zero lost responses across N=1..16.

```
HOP-TABLE
point                                                                resp/s    p50ms    p95ms     p99ms   lost  retry  srvCPU%   proc%  cliCPU%
-----------------------------------------------------------------------------------------------------------------------------------------------
GATE G direct engine→REAL cluster inflight=1024                     1751213    0.545    0.677     0.825      0      0     78.2   334.4     78.2
GATE abc remote→edge→REAL cluster conns=1 inflight=1024             1079930    0.935    1.144     1.269      0      0     76.0   181.4     33.3
GATE abc remote→edge→REAL cluster conns=2 inflight=1024             1359684    1.438    2.228     2.429      0      0     87.5   283.0     58.8
GATE abc remote→edge→REAL cluster conns=4 inflight=1024             1332151    2.898    3.994     4.686      0      0     85.4   274.8     79.0
GATE abc remote→edge→REAL cluster conns=8 inflight=1024             1277567    5.014    7.815     9.445      0      0     89.8   366.5     99.4
GATE abc remote→edge→REAL cluster conns=16 inflight=1024            1464381    9.241   13.730    16.974      0      0     87.4   384.1    100.0
GATE d engine→dummy-node engines=1 inflight=1024                    2442921    0.001    0.002     0.024      0      0     37.8   100.1     37.8
GATE d engine→dummy-node engines=2 inflight=1024                    3982214    0.511    0.521     0.538      0      0     62.6    99.9     62.5
GATE d engine→dummy-node engines=4 inflight=1024                    3568640    1.049    1.151     2.499      0      0    100.0    97.6    100.0
GATE d engine→dummy-node engines=8 inflight=1024                    3860151    0.520    0.809     1.542   6144      0    100.0    99.7    100.0
```

One honest note on row d: the engines=8 rung shows `lost=6144` — that is the
dummy-node arm at maximum oversubscription (8 engines + node + driver on an
8-vCPU host), ~0.016% of its ~38.6M sends, and row d adjudicates throughput
degradation (it scales up: 2.44 -> 3.98 -> 3.57 -> 3.86 M/s), not lost=0. Row c
(the connection ladder that must not collapse) held 0 lost at every rung.

Fleet destroyed after the run (`terraform state list` -> 0 resources).
