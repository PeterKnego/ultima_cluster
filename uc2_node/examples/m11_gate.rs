...
//! demand — see `examples/uc2-crashtest/tests/enospc.rs`'s doc for why
...
fn run_survival_suite_once(run_n: usize) -> SuiteRun {
    println!(
        "  [survival suite] run {run_n}/3: cargo test -p uc2-crashtest --features \
...
fn run_backup_straddle_test() -> (bool, String) {
    println!("  [row 1] straddle anti-vacuity: cargo test -p uc2_node --test backup a_wrong_order ...");
    let (exit_ok, combined) = cargo_test(&["test", "-p", "uc2_node", "--test", "backup", "a_wrong_order"]);
...
fn run_enospc_3a() -> Verdict {
    ...
    let (exit_ok, combined) =
        cargo_test(&["test", "-p", "uc2-crashtest", "--features", "enospc-tests", "write_denied"]);
...
/// Boundary-matched family presence — same rationale as `uc2_node/src/obs/metrics.rs`'s
...
/// deadlock the child (same concern `examples/uc2-crashtest/tests/enospc.rs`'s
...
/// the only writer — see `uc2_node/src/bin/uc2-node.rs`'s `free_disk_bytes`
...
fn run_flagday_smoke(root: &Path) {
    ...
    let node_bin = uc2_node_release_bin();
...
fn repo_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).parent().expect("uc2_node has a parent dir").to_path_buf()
}
...
/// --message-format=json` scan `examples/uc2-crashtest/tests/enospc.rs` uses
...
fn uc2_node_debug_bin() -> PathBuf {
    static CACHE: OnceLock<PathBuf> = OnceLock::new();
    build_bin(&CACHE, "uc2_node", "uc2-node", false)
}

fn uc2_node_release_bin() -> PathBuf {
    static CACHE: OnceLock<PathBuf> = OnceLock::new();
    build_bin(&CACHE, "uc2_node", "uc2-node", true)
}

fn uc2ctl_release_bin() -> PathBuf {
    static CACHE: OnceLock<PathBuf> = OnceLock::new();
    build_bin(&CACHE, "uc2ctl", "uc2ctl", true)
}
