...
//! (`uc2_gateway/src/bin/uc2-gateway.rs`) with one difference: it is
...
fn main() -> ExitCode {
    ...
    if let Err(e) = cfg.validate() {
        eprintln!("uc2-crashtest-gateway: {e}");
    ...
    let edge = match Edge::start(cfg) {
        Ok(e) => e,
        Err(e) => {
            eprintln!("uc2-crashtest-gateway: failed to start: {e}");
            return ExitCode::from(1);
        }
    };
    println!("uc2-crashtest-gateway: listening on {}", edge.local_addr());

    loop {
        if edge.is_faulted() {
            let s = edge.stats();
            eprintln!(
                "uc2-crashtest-gateway: edge faulted (the node's instance restarted); exiting 1 \
