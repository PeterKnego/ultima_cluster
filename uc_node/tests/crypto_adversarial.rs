...
//!
//! - `uc_node/tests/crypto_cluster.rs` (T17): the payoff capstone — a real
...
//!   file rather than exporting a shared test-only crate (`uc_node/src/
//!   node.rs`'s own T12 module doc says as much), so this follows the same
//!   convention.
//! - `uc_crypto`'s unit tests (esp. `handshake.rs`) cover forged/truncated/
//!   replayed inputs at the PRIMITIVE level (`Peers::on_message` fed
//!   fabricated bytes directly). This file never calls into `uc_crypto`
...
//! - `uc_net`'s receiver tests (esp. `receiver.rs`'s crypto module) cover
...
//!   [`Node`] wires up.
//! - `uc_net/src/fault.rs`'s new `corrupt_per_million`/`replay_per_million`
...
fn heavy_corruption_and_replay_injection_never_panics_and_never_diverges() {
    ...
    let mut svc_holder: Option<uc_service::Service<CountSm>> = None;
    ...
    fn read_frame_with_retry(n: &Node, pos: u64) -> (uc_protocol::v2::frame::FrameHeader, Vec<u8>) {
        ...
        loop {
            ...
            match n.read_frame_validated(pos, &mut out) {
                uc_log::buffer::FrameRead::Frame(h) => return (h, out),
                other => {
                    assert!(
                        Instant::now() < deadline,
