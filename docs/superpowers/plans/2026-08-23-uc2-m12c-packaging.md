...
- `cargo clippy --workspace --all-targets -- -D warnings` (+ `-p uc2_service --features ultima_db`, `--features apply-profile`, `-p uc2_gateway --features test-util`) clean and `cargo test --workspace --exclude uc2_node` + the `uc2_node` fast set green after every task; `Cargo.lock` committed.
...
| `examples/counter/src/bin/counter-remote.rs` | remote client example over `uc2_remote::RemoteClient` |
...
**Files:**
- Modify: root `Cargo.toml` (`[workspace.package]`, `[workspace.dependencies]`), every member `Cargo.toml` listed in the fact table below, `ultima_journal/Cargo.toml`
- Create: `deny.toml`
...
- `ultima_journal/Cargo.toml`: switch its literal `version`/`edition`/`license`/`authors` to `.workspace = true` (crate name stays `ultima-journal`).
- `description` added where missing: `uc2_node` ("The ultima_cluster node: four single-writer polling agents, the cnc page, elections, and the uc2-node daemon"), `uc2_service` ("Service-side SDK: RawStateMachine / StateMachine, Sessioned, snapshots, the apply agent"), `uc-lincheck` (test lib — stays unpublished but give it one), `examples/*` (unpublished; optional).
- Every internal path dep across the workspace gains `version = "2.6.0"` (`{ path = "../x", version = "2.6.0" }`), including `[workspace.dependencies].ultima-journal` and the `[dev-dependencies]` path entries (cargo requires versions there too for publish).
- `publish = false` stays on `uc-lincheck`, `examples/counter`, `examples/uc2-crashtest`; **added** to `uc2_sim` (simulation harness, not in the published DAG); **removed** from `uc2ctl`.
- Remove from `[workspace.dependencies]`: `quinn`, `rustls`, `rcgen`, `rustls-pemfile`, `tokio`, `futures` (none referenced by any member; `uc2_service` keeps its own narrow `tokio`); delete the "QUIC + TLS (M2)" comment block. After `cargo update -w`/`cargo build`, assert `cargo tree -i ring` and `cargo tree -i quinn` print nothing.
- `deny.toml`:
  ```toml
  [graph]
  all-features = false
  ...
  unknown-git = "deny"
  ```
- `ci.yml` gains jobs `deny` (`EmbarkStudios/cargo-deny-action@v2` with `command: check advisories licenses bans sources`) and `publish-check` (steps: `cargo package --no-verify --allow-dirty -p <crate>` for each of the 12 publishable crates in DAG order, then `cargo publish --dry-run -p ultima-journal`, `-p uc_protocol`, `-p uc2_remote`, `-p uc2_consensus` — the leaves, the only ones resolvable before anything is on crates.io; a comment explains why the rest are validated by `cargo package` only and by the real publish order in `docs/how-to/cut-a-release.md`).

- [ ] **Step 1: Failing checks first.** `cargo tree -i ring | head -1` (expect output today — `ring` comes in via the dead `rustls`/`quinn`), `cargo package --no-verify --allow-dirty -p uc2_log 2>&1 | tail -3` (expect "all dependencies must have a version specified when publishing" → this is the RED), and `cargo install cargo-deny` (if not present) + `cargo deny check 2>&1 | tail -5` (expect failures: no config / licenses).
...
- Modify: `examples/counter/Cargo.toml` (+ `[[bin]] counter-remote`, dep `uc2_remote`, feature `quickstart-tests = []`), `.github/workflows/nightly.yml` (+ `quickstart` job running that test), `examples/counter/src/lib.rs` docs if the Command/Query types need `pub` re-exports for the remote bin

**Interfaces:**
- `counter-remote --gateways HOST:PORT[,HOST:PORT…] --app-id A add <n> | reset | get [--linearizable]` — encodes `counter::Command` with bincode-standard into opaque bytes (exactly as `counter-client` does), submits via `uc2_remote::RemoteClient` (`RemoteConfig { app_id, members, ..Default::default() }`), decodes `counter::Applied`/`QueryResponse`, prints `value=<v> position=<p> replayed=<bool>`; exit 0 on success, 1 on error, 2 on bad args; `--timeout-secs` (default 10). SPDX header; doc comment explains it is the remote twin of `counter-client` and the reference for "how a remote client talks to a gateway".
...
- `packaging/Dockerfile`: `FROM gcr.io/distroless/cc-debian12`; `ARG TARGETARCH`; `COPY uc2-${VERSION}-${arch}/bin/ /usr/local/bin/` (map amd64→x86_64, arm64→aarch64 in a tiny pre-stage or pass `--build-arg`), `COPY packaging/ /opt/uc2/packaging/`; `USER 65532`; `ENTRYPOINT ["/usr/local/bin/uc2-node"]`; a comment on how to run uc2ctl/uc2-gateway/counter-service from the same image (`--entrypoint`).
...
- §3.2 lockstep 2.6.0, path-dep versions, `ultima_journal` joins, `uc2ctl` publishable, examples/`uc-lincheck` not, `rust-version` + pin + `msrv` job, `semver-policy.md` → Tasks 1, 2, 5 (+ `uc2_sim` unpublished — plan addition, ruled). ✔
