...
| Reserved FSM name prefix | **`uc_`** — a `[services] names` entry starting with it is a startup refusal. UC's cluster FSM is `uc_cluster` | the cluster FSM (2.11 pending) — [the cluster FSM explainer](../notes/uc2-cluster-fsm-explained.md) |
| Cluster-wide settings that are replicated | **four**: `fsm_lag`, `admission_bytes`, `snapshot_interval_bytes`, `snapshot_target`. `[settings]` in `node.toml` seeds genesis only; the moved per-host spellings (top-level `admission_bytes`, `[services] fsm_lag`) are startup refusals by name | the cluster FSM (2.11 pending) — [Configuration § `[settings]`](configuration.md#settings), [`uc2ctl` § `settings apply`](uc2ctl.md#settings-apply) |
...
| Entries in the replicated schedule table | **32** (`MAX_SCHEDULE_ENTRIES`, a source constant, no knob), across every FSM. A full table is `8 + 32 × 33` = **1064 B**, which is why it always fits one datagram | the schedule table (2.11 pending) — [Wire protocol § `ScheduleTable` payload](wire-protocol.md#scheduletable-payload-cluster-kind-2-wire-070) |
...
| Cluster commands in flight | **1**, across all three kinds — the leader answers `retry` (status 2) while any previous `CLUSTER` frame (a membership change, a schedule table or a settings record) is still above the committed cluster view. A `settings apply` therefore retries behind an in-flight membership change and vice versa | the cluster FSM (2.11 pending) — [`uc2ctl` § Sub-commands](uc2ctl.md#sub-commands) |
...
| Snapshot artifacts per set | **one per declared row plus one for the cluster FSM**, all tagged with the **same** position P. A session ships one set or none; `SNAP_BEGIN`s that disagree about `snapshot_pos` are refused | coordinated snapshots (2.11 pending) — [Wire protocol § One session, one position](wire-protocol.md#one-session-one-position-wire-070) |
| Snapshot instants in flight | **1** per leader. A leader appends no new `SNAPSHOT` while its own set at the last commanded P is incomplete, unless a further `snapshot_interval_bytes` of log has accrued (which **abandons** the old instant) or an operator's `uc2ctl snapshot` supersedes it, which it always does | coordinated snapshots (2.11 pending) — [`uc2ctl` § `snapshot`](uc2ctl.md#snapshot) |
| `snapshot fetch` position | **48 bits** — the position rides the admin request's two free address words. `uc2ctl` refuses a `--position` at or above `1 << 48` by name rather than truncating | coordinated snapshots (2.11 pending) — [`uc2ctl` § `snapshot fetch`](uc2ctl.md#snapshot-fetch) |
| Snapshot intake deadline | **60 s** (`SNAP_INTAKE_TIMEOUT_NS`, a source constant, no knob) — one deadline for an inbound session and for a pending `snapshot fetch` alike. Past it the intake is abandoned, its `.part` files unlinked, and a straggling answer is refused `fetch_expired` | coordinated snapshots (2.11 pending) |
