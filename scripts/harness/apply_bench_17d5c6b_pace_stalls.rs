// GATE ROW d — BACKPORTED HARNESS OVERLAY for `scripts/apply_ab.sh --harness-a`.
//
// This is `uc_node/examples/apply_bench.rs` AS OF 17d5c6b (the pre-time-and-timers
// baseline that row d's pair names) with ONE change: its `APPLY-JSON` line now
// carries the `pace_stalls` field. Nothing else differs — diff it against
// `git show 17d5c6b:uc_node/examples/apply_bench.rs` to confirm.
//
// WHY THIS FILE EXISTS. Row d A/Bs `17d5c6b` -> `HEAD`, and Ruling Q9's
// driver-bound guard — which the gate doc calls "the ONLY guard that can tell
// row d's arms apart" — reads `pace_stalls`. The counter was added to the
// harness on 2026-09-07, AFTER the baseline commit, so the base arm emitted no
// such field and the guard could not be evaluated at all. On the 2026-09-07/08
// fleet run that made row d unadjudicable: it read +7.561 % (N=1) and +6.554 %
// (N=2) with `pace_stalls_unknown_arms = ["A"]`.
//
// The pair straddles the `Appender::new`/`append` arity change, so `--harness`
// (one source on every arm) cannot compile on both sides — which is why this is
// a `--harness-a` overlay, applied to the BASE arm only.
//
// WHY THE BACKPORT IS SAFE. The counter is not new logic: `17d5c6b` already
// incremented `stalls` in its pacing loop, and that loop is BYTE-IDENTICAL to
// HEAD's —
//
...
//     }
//
// — so both arms count the same event at the same point. Only the reporting
// changed; the baseline already printed the value on its human-readable line
// (`driver appended_frames/s=... pace_stalls=...`), it simply never reached the
// machine-readable one.
//
// The harness ASYMMETRY that row d has by construction is unchanged and still
// has to be disclosed when quoting the row: each arm builds a different driver.
// What this file buys is that the guard can now say whether that driver was the
// limiter — which is the difference between a reading and a verdict.
...
fn main() -> anyhow::Result<()> {
    ...
    let per_json: Vec<String> = per
        ...
        .collect();
    println!(
        "APPLY-JSON {{\"fsms\":{},\"mode\":\"{}\",\"lag\":{},\"payload\":{},\"frame\":{},\"secs\":{:.2},\"min_rate\":{:.0},\"driver_rate\":{:.0},\"pace_stalls\":{},\"per\":[{}]}}",
        a.fsms,
        a.mode,
        lag,
        a.payload,
